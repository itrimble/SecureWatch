...
const AuthenticationDashboard: React.FC = () => {
  ...
  const [topFailedLoginUsers, setTopFailedLoginUsers] = useState<TopFailedLoginUser[]>([]);
  const [topFailedLoginUsersLoading, setTopFailedLoginUsersLoading] = useState(true);
  const [topFailedLoginUsersError, setTopFailedLoginUsersError] = useState<string | null>(null);

  const [authAttemptsChartData, setAuthAttemptsChartData] = useState<AuthAttemptOverTimeChartData[]>([]);
  const [authAttemptsChartLoading, setAuthAttemptsChartLoading] = useState(true);
  const [authAttemptsChartError, setAuthAttemptsChartError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/dashboards/authentication')
      .then(res => {
        if (!res.ok) {
          throw new Error(`Failed to fetch main authentication data: ${res.status} ${res.statusText}`);
        }
        return res.json();
      })
      .then(fetchedData => {
    ...
    fetch('/api/query', {
      ...
      body: JSON.stringify({
        query: 'events | where event_type_id == "4625" | summarize attempts = count() by user_id | sort by attempts desc | take 10'
      }),
    })
    .then(res => {
      if (!res.ok) {
        return res.json().then(errBody => {
            throw new Error(`KQL query for top failed logins failed: ${res.statusText} - ${errBody.error || errBody.details || 'Unknown error'}`);
        }).catch(() => { throw new Error(`KQL query for top failed logins failed: ${res.statusText}`); });
      }
      return res.json();
    })
    .then(queryResult => {
      if (queryResult.error) throw new Error(queryResult.error.details || queryResult.error);
    ...
    fetch('/api/query', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        query: "events | summarize count_ = count() by timestamp_hour = date_trunc('hour', timestamp), event_source_name | sort by timestamp_hour asc"
      })
    })
    .then(res => {
      if (!res.ok) {
        return res.json().then(errBody => {
            throw new Error(`KQL for auth attempts failed: ${res.statusText} - ${errBody.error || errBody.details || 'Unknown error'}`);
        }).catch(() => { throw new Error(`KQL for auth attempts failed: ${res.statusText}`); });
      }
      return res.json();
    })
    .then(queryResult => {
      ...
      rawData.forEach(item => {
        const formattedTime = new Date(item.timestamp_hour).toLocaleString([], { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
        ...
        if (seriesName === "Windows Security" || seriesName === "Microsoft-Windows-Security-Auditing") seriesName = "Windows";
  ...
  const pageLoading = loadingMain || topFailedLoginUsersLoading || authAttemptsChartLoading;

  if (pageLoading && !data) return <div className="text-center p-8 text-gray-100">Loading authentication data...</div>;
  // Display general error if main data fetch failed, otherwise individual widgets will show their errors.
  if (errorMain && !data) return <div className="text-center p-8 text-red-500">Error loading critical dashboard data: {errorMain}</div>;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 p-4 md:p-6">
      ...
      {data ? (
        <>
         ...
         !loadingMain && <div className="lg:col-span-2 text-gray-400 p-4">Main metrics data unavailable. {errorMain}</div>
      )}
      ...
      <div className="lg:col-span-3 bg-gray-800 p-4 md:p-6 rounded-lg shadow-xl h-96 md:h-[500px]">
        ...
        {loadingMain && <p className="text-gray-300 text-center pt-10">Loading Geo Map data...</p>}
        {!loadingMain && data && data.loginAttemptsGeo && data.loginAttemptsGeo.length > 0 ? (
          <ReactECharts
            option={{
              ...
              visualMap: { /* ... existing visualMap ... */
                min: 0,
                max: data.loginAttemptsGeo.length > 0 ? data.loginAttemptsGeo.reduce((maxVal, item) => Math.max(maxVal, item.value[0] + item.value[1]), 0) : 0,
                left: 'left',
                top: 'bottom',
                text: ['High', 'Low'],
                calculable: true,
              ...
              series: [ /* ... existing series ... */
                {
                  name: 'Login Attempts', type: 'map', map: 'world', roam: true,
                  emphasis: { label: { show: true, color: '#fff' }, itemStyle: { areaColor: '#A9D0F5' } },
                  itemStyle: { areaColor: '#323c48', borderColor: '#111' },
                  data: data.loginAttemptsGeo.map(item => ({
                    name: item.name, value: item.value[0] + item.value[1],
                    originalSuccessful: item.value[0], originalFailed: item.value[1]
                  })),
                }
              ],
          ...
          !loadingMain && <p className="text-gray-400 text-center pt-10">No geo-map data available. {errorMain && `Error: ${errorMain}`}</p>
        )}
      </div>
      ...
      <div className="lg:col-span-2 bg-gray-800 p-4 md:p-6 rounded-lg shadow-xl h-96">
        ...
        {authAttemptsChartLoading && <p className="text-gray-300 text-center pt-10">Loading chart data...</p>}
        {authAttemptsChartError && <p className="text-red-500 text-center pt-10">Error: {authAttemptsChartError}</p>}
        {!authAttemptsChartLoading && !authAttemptsChartError && authAttemptsChartData.length > 0 ? (
          <ResponsiveContainer width="100%" height="85%">
            <LineChart data={authAttemptsChartData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#4b5563" />
              <XAxis dataKey="timestamp_hour" stroke="#9ca3af" tickFormatter={(timeStr) => new Date(timeStr).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} />
              <YAxis stroke="#9ca3af" />
              <Tooltip
              ...
              {authChartSeriesKeys.map(key => (
                <Line key={key} type="monotone" dataKey={key} stroke={lineColors[key] || lineColors["Other"]} name={key} dot={false} />
              ))}
            </LineChart>
          </ResponsiveContainer>
        ) : (
           !authAttemptsChartLoading && <p className="text-gray-400 text-center pt-10">No authentication attempt data available.</p>
        )}
      </div>
      ...
      {data ? (
        ...
        !loadingMain && <div className="lg:col-span-1 text-gray-400 p-4">Success/Failure by Application data unavailable. {errorMain}</div>
      )}
      ...
      <div className="lg:col-span-3 bg-gray-800 p-4 md:p-6 rounded-lg shadow-xl h-96 md:h-[500px]">
        ...
        {loadingMain && <p className="text-gray-300 text-center pt-10">Loading Heatmap data...</p>}
        {!loadingMain && data && data.peakAuthTimesHeatmap && data.peakAuthTimesHeatmap.length > 0 ? (
          <ResponsiveHeatMap
            data={data.peakAuthTimesHeatmap}
            indexBy="id"
            keys={data.peakAuthTimesHeatmap[0] && data.peakAuthTimesHeatmap[0].data ? data.peakAuthTimesHeatmap[0].data.map(d => d.x) : []}
            margin={{ top: 70, right: 90, bottom: 60, left: 90 }}
            axisTop={{ /* ... */ }}
            axisLeft={{ /* ... */ }}
            colors={{ type: 'sequential', scheme: 'inferno' }}
            cellOpacity={0.9} cellBorderWidth={1}
            ...
            tooltip={({ cell }) => (
              <div className="p-2 bg-gray-900 text-white rounded shadow-lg border border-gray-700">
                <strong>{cell.serieId}</strong> at <strong>{cell.label || cell.xKey}</strong>
                <br />
          ...
          !loadingMain && <p className="text-gray-400 text-center pt-10">No peak time heatmap data available. {errorMain && `Error: ${errorMain}`}</p>
        )}
      </div>
      ...
      <div className="lg:col-span-3 bg-gray-800 p-4 md:p-6 rounded-lg shadow-xl">
        ...
        {topFailedLoginUsersLoading && <p className="text-gray-300">Loading top failed login users...</p>}
        {topFailedLoginUsersError && <p className="text-red-500">Error loading users: {topFailedLoginUsersError}</p>}
              ...
              <table className="w-full text-sm text-left text-gray-400">
                <thead className="text-xs text-gray-100 uppercase bg-gray-700">
                  <tr>
                    ...
                    <th scope="col" className="py-3 px-6 text-right">Attempts</th>
                  </tr>
                </thead>
                <tbody>
                  {topFailedLoginUsers.map((user, index) => (
                    <tr key={index} className="bg-gray-800 border-b border-gray-700 hover:bg-gray-600">
                      ...
                      <td className="py-4 px-6 text-right">{user.attempts.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ) : (
              <p className="text-gray-400">No data on users with high failed login attempts.</p>
            )}
          </div>
        )}
      </div>
