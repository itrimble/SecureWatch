...
export class EnhancedDashboardUI {
  ...
  private activePanel: string = 'microservices';
  ...
  private navigateServices(direction: number) {
    ...
    this.selectedServiceIndex = Math.max(0, 
      Math.min(allServices.length - 1, this.selectedServiceIndex + direction)
    );
    
    this.updateServiceDetails(allServices[this.selectedServiceIndex]);
  }

  private updateServiceDetails(serviceName: string) {
...
{bold}Last Check:{/bold} ${moment(service.lastChecked).format('HH:mm:ss')}
...
`;
  ...
  update(data: DashboardData) {
    ...
    this.updateBottomBar(`Last updated: ${moment(data.lastUpdated).format('HH:mm:ss')} | Active Panel: ${this.activePanel} | Press 'h' for help`);
...
`;
  ...
  private updateMicroservices(services: ServiceStatus[]) {
    const microservices = services.filter(s => 
      this.serviceCategories.microservices.includes(s.name)
    );
  ...
  private updateInfrastructure(services: any[]) {
  ...
  private updateBottomBar(message: string) {
  ...
  private getHealthColor(percentage: number): string {
    if (percentage >= 90) return '{green-fg}';
    if (percentage >= 70) return '{yellow-fg}';
    return '{red-fg}';
  }
