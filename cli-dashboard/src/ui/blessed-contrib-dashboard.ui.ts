...
export class BlessedContribDashboardUI {
  private screen: blessed.Widgets.Screen;
  private grid: any;
  private widgets: any = {};
  private currentData: DashboardData | null = null;
  private controlService: ServiceControlService;
  private fontDetector: FontDetector;
  private activePanel: string = 'services';
  private selectedServiceIndex: number = 0;
  private viewMode: 'all' | 'critical' | 'compact' = 'all';
  
  // Historical data for charts
  private cpuHistory: number[] = [];
  private memoryHistory: number[] = [];
  private diskHistory: number[] = [];
  private networkInHistory: number[] = [];
  private networkOutHistory: number[] = [];
  private epsHistory: number[] = [];
  private alertsHistory: number[] = [];
  ...
  private initializeHistoricalData() {
    ...
    for (let i = 0; i < dataPoints; i++) {
      ...
      this.timestamps.push(moment().subtract(dataPoints - i, 'minutes').format('HH:mm'));
    }
  }
  ...
  private getIcon(type: string): string {
    if (!this.fontDetector.hasNerdFonts()) {
      ...
      const asciiIcons: { [key: string]: string } = {
        services: '[*]',
        trend: '[~]',
        rules: '[R]',
        cache: '[C]',
    ...
    const nerdIcons: { [key: string]: string } = {
  ...
  private updateTopStatusBar(data: DashboardData) {
...
{center}${healthIcon} Health: {green-fg}${overallHealth.toFixed(0)}%{/} | Services: {cyan-fg}${operationalServices}/${totalServices}{/} | Alerts: {red-fg}${criticalAlerts}{/} Critical | Mode: {yellow-fg}${this.viewMode.toUpperCase()}{/} | Font: {magenta-fg}${this.fontDetector.hasNerdFonts() ? 'Enhanced' : 'Standard'}{/}{/center}`;
  ...
  private updateServiceStatusPanel(services: ServiceStatus[]) {
  ...
  private getCategoryIcon(serviceName: string): string {
    if (!this.fontDetector.hasNerdFonts()) return '•';
    
    if (this.serviceCategories.core.includes(serviceName)) return '';
    if (this.serviceCategories.auth.includes(serviceName)) return '';
    if (this.serviceCategories.ui.includes(serviceName)) return '';
    if (this.serviceCategories.storage.includes(serviceName)) return '';
    if (this.serviceCategories.messaging.includes(serviceName)) return '';
    if (this.serviceCategories.monitoring.includes(serviceName)) return '';
    if (this.serviceCategories.agents.includes(serviceName)) return '';
    
    return '';
  }

  private updatePlatformMetricsPanel(data: DashboardData) {
  ...
  private updateSystemResourcesPanel(resources: any) {
  ...
  private updateRecentActivityPanel(alerts: any[], logs: any[]) {
  ...
  private updateHistoricalData(data: DashboardData) {
    ...
    this.epsHistory.push(Math.random() * 1000 + 500);
    this.alertsHistory.push(Math.floor(Math.random() * 5));
    this.timestamps.push(moment().format('HH:mm'));
...
{bold}Current Mode:{/bold} {yellow-fg}${this.viewMode.toUpperCase()}{/}`;
  ...
  private updateBottomStatusBar(message: string = '') {
    const defaultMessage = `SecureWatch CLI Dashboard v2.0 | ${this.fontDetector.hasNerdFonts() ? 'Enhanced' : 'Standard'} Mode | Last Update: ${moment().format('HH:mm:ss')}`;
    this.widgets.bottomStatusBar.setContent(`{center}${message || defaultMessage}{/center}`);
  }

  // Service Control Methods
  private navigateServices(direction: number) {
    if (!this.currentData?.services) return;
    
    this.selectedServiceIndex = Math.max(0, Math.min(
      this.currentData.services.length - 1,
      this.selectedServiceIndex + direction
    ));
  ...
  private async startSelectedService() {
  ...
  private async stopSelectedService() {
    const service = this.currentData?.services[this.selectedServiceIndex];
    if (service) {
  ...
  private async restartSelectedService() {
    const service = this.currentData?.services[this.selectedServiceIndex];
    if (service) {
  ...
  private async restartAllServices() {
  ...
  private setViewMode(mode: 'all' | 'critical' | 'compact') {
    this.viewMode = mode;
...
Press ESC or 'q' to close this help.`;
  ...
  private showMessage(message: string, type: 'info' | 'warning' | 'error' | 'success' = 'info', duration: number = 3000) {
    const colors = {
      info: 'blue',
      warning: 'yellow', 
      error: 'red',
