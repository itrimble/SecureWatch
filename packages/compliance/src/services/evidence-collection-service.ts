...
export class EvidenceCollectionService extends EventEmitter {
  ...
  private collectionJobs: Map<string, CollectionJob> = new Map();
  ...
  async initialize(): Promise<void> {
    logger.info('Initializing Evidence Collection Service');
    ...
    this.registerCollector({
      ...
      collect: async (config) => {
        ...
        try {
          ...
          logger.error('API collection failed:', error);
          throw error;
        }
      },
    ...
    this.registerCollector({
      ...
      collect: async (config) => {
        ...
        try {
          ...
          logger.error('Database query collection failed:', error);
          throw error;
        }
      }
    });
    ...
    this.registerCollector({
      ...
      collect: async (config) => {
        ...
        try {
          ...
          logger.error('File system collection failed:', error);
          throw error;
        }
      }
    });
  ...
  registerCollector(collector: CollectorPlugin): void {
    ...
    logger.info(`Registered collector: ${key}`);
  }

  // Evidence Collection Rule Management
  async createCollectionRule(ruleData: Omit<EvidenceCollectionRule, 'id'>): Promise<EvidenceCollectionRule> {
    const now = new Date();
    const newRule: EvidenceCollectionRule = {
      id: uuidv4(),
      ...ruleData
    };

    const validatedRule = EvidenceCollectionRuleSchema.parse(newRule);
    ...
    logger.info(`Created evidence collection rule: ${validatedRule.name}`);

    return validatedRule;
  }

  async updateCollectionRule(ruleId: string, updates: Partial<EvidenceCollectionRule>): Promise<void> {
    ...
    const validatedRule = EvidenceCollectionRuleSchema.parse(updated);
  ...
  async getCollectionRule(ruleId: string): Promise<EvidenceCollectionRule | null> {
  ...
  async getCollectionRules(frameworkId?: string, active?: boolean): Promise<EvidenceCollectionRule[]> {
  ...
  // Evidence Collection
  async collectEvidence(request: EvidenceCollectionRequest): Promise<ComplianceEvidence> {
  ...
  async runCollectionRule(ruleId: string): Promise<CollectionJob> {
  ...
  private async executeCollection(rule: EvidenceCollectionRule, job: CollectionJob): Promise<void> {
    ...
    try {
      ...
      logger.error(`Collection failed for rule ${rule.id}:`, error);
    }
  }
  ...
  private async createEvidence(
    ...
    if (existing) {
      logger.info(`Evidence already exists with hash: ${hash}`);
    ...
    const validatedEvidence = ComplianceEvidenceSchema.parse(evidence);
    ...
    logger.info(`Created evidence: ${validatedEvidence.id} (${validatedEvidence.type})`);
    return validatedEvidence;
  }

  private async mapEvidenceToControls(
    evidenceId: string,
    frameworkId: string,
    controlIds: string[]
  ): Promise<void> {
  ...
  private validateEvidence(data: any, rules: any[]): boolean {
  ...
  async searchEvidence(
    ...
    const evidence = rows.map((row: any) => this.parseEvidenceRow(row));
  ...
  async getEvidenceForControl(
    ...
    return rows.map((row: any) => this.parseEvidenceRow(row));
  }

  // Scheduling
  private async loadCollectionRules(): Promise<void> {
  ...
  private scheduleCollection(rule: EvidenceCollectionRule): void {
    if (!rule.automation.schedule) return;
    ...
    if (schedule === '0 * * * *') { // Every hour
      ...
      logger.warn(`Unsupported schedule format: ${schedule}`);
      return;
    }
    ...
    logger.info(`Scheduled collection for rule ${rule.id}: ${schedule}`);
  }

  private cancelScheduledCollection(ruleId: string): void {
    const timeout = this.scheduledJobs.get(ruleId);
    if (timeout) {
      clearInterval(timeout);
      this.scheduledJobs.delete(ruleId);
      logger.info(`Cancelled scheduled collection for rule ${ruleId}`);
    }
  }

  private async updateNextRunTime(ruleId: string): Promise<void> {
    const rule = await this.getCollectionRule(ruleId);
    if (!rule || !rule.automation.schedule) return;
  ...
  private async recordCollectionHistory(
    ruleId: string,
    job: CollectionJob,
  ...
  private parseEvidenceRow(row: any): ComplianceEvidence {
    return {
      id: row.id,
      type: row.type,
      source: row.source,
  ...
  async cleanupExpiredEvidence(): Promise<number> {
    ...
    if (result > 0) {
      logger.info(`Cleaned up ${result} expired evidence records`);
  ...
  async shutdown(): Promise<void> {
    logger.info('Shutting down Evidence Collection Service');
