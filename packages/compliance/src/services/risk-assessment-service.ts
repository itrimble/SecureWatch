...
interface RiskCalculationFactors {
...
export class RiskAssessmentService extends EventEmitter {
  ...
  private loadDefaultThreatScenarios(): void {
  ...
  async assessFrameworkRisk(
    ...
    const controlRisks: ComplianceRisk[] = [];
  ...
  private async assessControlRisk(
    frameworkId: string,
  ...
  private calculateLikelihood(control: ComplianceControl, status: ComplianceStatus): number {
  ...
  private calculateImpact(control: ComplianceControl): number {
    // Base impact on control weight
    const weight = control.riskWeight;

    if (weight >= 9) return 5; // Critical impact
    if (weight >= 7) return 4; // Major impact
    if (weight >= 5) return 3; // Moderate impact
    if (weight >= 3) return 2; // Minor impact
  ...
  private calculateAdjustedRiskScore(
    ...
    likelihood: number,
    impact: number,
    factors: RiskCalculationFactors
  ): number {
  ...
  private async getRiskFactors(): Promise<RiskCalculationFactors> {
  ...
  private async generateRecommendations(
    ...
    for (const risk of highRisks) {
      ...
      const recommendation: RiskRecommendation = {
        ...
        priority: risk.riskLevel as RiskLevel,
  ...
  private generateRecommendationDescription(risk: ComplianceRisk, control: ComplianceControl): string {
    const descriptions = {
  ...
  private estimateEffort(risk: ComplianceRisk, control: ComplianceControl): string {
  ...
  private estimateCost(risk: ComplianceRisk, control: ComplianceControl): number {
    let baseCost = 10000;
  ...
  private generateMitigationSteps(risk: ComplianceRisk, control: ComplianceControl): string[] {
    const steps: string[] = [];
  ...
  private isQuickWin(risk: ComplianceRisk, control?: ComplianceControl): boolean {
  ...
  async addMitigation(
    riskId: string,
    mitigation: Omit<ComplianceRisk['mitigations'][0], 'id'>
  ): Promise<void> {
    const risk = await this.getRisk(riskId);
    if (!risk) {
  ...
  async getRiskMatrix(frameworkId: string): Promise<RiskMatrix[]> {
    ...
    const matrix: Map<string, RiskMatrix> = new Map();
  ...
  async getHighRiskControls(frameworkId: string, limit: number = 10): Promise<ComplianceRisk[]> {
    ...
    return rows.map((row: any) => this.parseRiskRow(row));
  }

  // Data Persistence
  private async saveRisk(risk: ComplianceRisk): Promise<void> {
    const now = new Date();
  ...
  private async saveAssessment(assessment: RiskAssessmentResult): Promise<void> {
  ...
  private async updateRiskTrends(
    frameworkId: string,
  ...
  // Data Retrieval
  async getRisk(riskId: string): Promise<ComplianceRisk | null> {
  ...
  async getRiskByControl(frameworkId: string, controlId: string): Promise<ComplianceRisk | null> {
  ...
  private parseRiskRow(row: any): ComplianceRisk {
    return {
      id: row.id,
  ...
  async shutdown(): Promise<void> {
    ...
    logger.info('Risk Assessment Service shutdown complete');
  }
}