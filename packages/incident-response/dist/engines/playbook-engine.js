...
class PlaybookEngine extends events_1.EventEmitter {
    constructor(config) {
        super();
    ...
    async initialize() {
        logger_1.logger.info('Initializing Playbook Engine');
        await this.createTables();
        await this.loadActivePlaybooks();
        await this.resumePendingExecutions();
        logger_1.logger.info('Playbook Engine initialized successfully');
    }
    async createTables() {
        // Playbooks table
    ...
    async createPlaybook(playbookData) {
        ...
        const validatedPlaybook = incident_response_types_1.PlaybookSchema.parse(playbook);
        ...
        logger_1.logger.info(`Created playbook ${validatedPlaybook.id}: ${validatedPlaybook.name}`);
    ...
    async updatePlaybook(playbookId, updates) {
        ...
        const validated = incident_response_types_1.PlaybookSchema.parse(updated);
    ...
    async executePlaybook(playbookId, context) {
        ...
        const validatedContext = incident_response_types_1.ExecutionContextSchema.parse(executionContext);
        ...
        try {
            ...
            logger_1.logger.error(`Playbook execution ${validatedContext.executionId} failed:`, error);
            const result = {
                executionId: validatedContext.executionId,
                playbookId,
                status: 'failure',
    ...
    async executeSteps(playbook, context) {
        const startTime = Date.now();
        let stepsExecuted = 0;
        ...
        for (const step of sortedSteps) {
            ...
            if (step.condition && !this.evaluateCondition(step.condition, context)) {
                logger_1.logger.debug(`Step ${step.id} condition not met, skipping`);
                continue;
            }
            try {
                stepsExecuted++;
                ...
                if (stepResult.success) {
                    ...
                    if (step.onSuccess) {
                        ...
                        logger_1.logger.debug(`Step ${step.id} succeeded, next: ${step.onSuccess}`);
                    }
                }
                else {
                    stepsFailed++;
                    ...
                    // Handle failure path
                    if (step.onFailure) {
                        logger_1.logger.debug(`Step ${step.id} failed, next: ${step.onFailure}`);
                    }
                ...
                if (step.timeout && stepResult.duration && stepResult.duration > step.timeout * 60 * 1000) {
                    logger_1.logger.warn(`Step ${step.id} exceeded timeout of ${step.timeout} minutes`);
                }
            }
            catch (error) {
                stepsFailed++;
                const errorMessage = error instanceof incident_response_types_1.PlaybookExecutionError ? error.message : error.message;
    ...
    evaluateCondition(condition, context) {
        ...
        switch (operator) {
            ...
            default:
                logger_1.logger.warn(`Unknown condition operator: ${operator}`);
                return false;
        }
    }
    async executeAction(action, context) {
        ...
        if (!handler) {
            throw new incident_response_types_1.PlaybookExecutionError(`No handler found for action type: ${action.type}`, 'unknown', context.executionId);
        }
        // Validate action if handler has validation
        if (handler.validate && !handler.validate(action)) {
            throw new incident_response_types_1.PlaybookExecutionError(`Action validation failed for type: ${action.type}`, 'unknown', context.executionId);
        }
        const startTime = Date.now();
        let result;
        try {
            ...
            // Retry logic
            if (action.retries > 0) {
                logger_1.logger.info(`Retrying action ${action.type}, attempts remaining: ${action.retries}`);
                const retryAction = { ...action, retries: action.retries - 1 };
                await new Promise(resolve => setTimeout(resolve, 1000)); // Wait 1 second before retry
                return this.executeAction(retryAction, context);
            }
        }
        return incident_response_types_1.ActionResultSchema.parse(result);
    }
    // Approval Workflow
    async requestApproval(playbook, context) {
    ...
    async approvePlaybook(executionId, approverId, comments) {
    ...
    async rejectPlaybook(executionId, approverId, comments) {
        const context = this.pendingApprovals.get(executionId);
        if (!context) {
            throw new Error(`No pending approval found for execution ${executionId}`);
        }
    ...
    async handleApprovalTimeout(executionId) {
        await this.db('playbook_executions')
            .where('id', executionId)
            .update({
            status: 'timeout',
    ...
    matchesTriggerConditions(conditions, triggerData) {
        if (!conditions)
            return false;
        // Check alert type
        ...
        if (conditions.tags && conditions.tags.length > 0) {
            ...
            if (!conditions.tags.some((tag) => triggerTags.includes(tag))) {
                return false;
            }
        }
        ...
        if (conditions.mitreAttackTechniques && conditions.mitreAttackTechniques.length > 0) {
            ...
            if (!conditions.mitreAttackTechniques.some((technique) => triggerTechniques.includes(technique))) {
                return false;
            }
        }
        return true;
    ...
    // Helper Methods
    async createExecutionRecord(context, playbook) {
    ...
    async updateExecutionRecord(executionId, result) {
        await this.db('playbook_executions')
            .where('id', executionId)
            .update({
    ...
    async recordStepExecution(executionId, step, status, result) {
        const record = {
            id: (0, uuid_1.v4)(),
    ...
    async updatePlaybookStatistics(playbookId, success) {
        const playbook = await this.getPlaybook(playbookId);
        if (!playbook)
            return;
    ...
    async getPlaybookName(playbookId) {
        const playbook = await this.getPlaybook(playbookId);
    ...
    async sendNotification(channel, data) {
        ...
        logger_1.logger.info(`Sending ${channel} notification:`, data);
        return { channel, status: 'sent', timestamp: new Date() };
    }
    async sendApprovalNotifications(playbook, context) {
    ...
    async loadActivePlaybooks() {
        const activePlaybooks = await this.db('playbooks').where('enabled', true);
        logger_1.logger.info(`Loaded ${activePlaybooks.length} active playbooks`);
    }
    async resumePendingExecutions() {
        ...
        logger_1.logger.info(`Resumed ${pendingExecutions.length} pending executions`);
    }
    mapRowToPlaybook(row) {
        return {
            id: row.id,
            name: row.name,
    ...
    async getPlaybookStatistics() {
        const stats = await this.db('playbooks')
            .select(this.db.raw('COUNT(*) as total'), this.db.raw('COUNT(CASE WHEN enabled = true THEN 1 END) as enabled'), this.db.raw('SUM(execution_count) as total_executions'), this.db.raw('AVG(success_rate) as avg_success_rate'))
            .first();
    ...
    async shutdown() {
        logger_1.logger.info('Shutting down Playbook Engine');
        ...
        logger_1.logger.info('Playbook Engine shutdown complete');
    }
}
