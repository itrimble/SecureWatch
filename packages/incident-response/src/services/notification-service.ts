...
export class NotificationService extends EventEmitter {
  ...
  constructor(config: { database: DatabaseConfig; irConfig: IRConfig }) {
    super();
    this.db = knex({
...
Execution ID: {{executionId}}`,
  ...
  async sendNotification(notificationData: Omit<Notification, 'id' | 'createdAt' | 'status'>): Promise<Notification> {
    ...
    if (userPreferences) {
      ...
      validatedNotification.channels = validatedNotification.channels.filter(channel => 
        userPreferences.enabledChannels.includes(channel)
      );

      // Check priority threshold
      const priorityLevels = { p4: 1, p3: 2, p2: 3, p1: 4 };
      const notificationPriority = priorityLevels[validatedNotification.priority as keyof typeof priorityLevels] || 1;
      const thresholdPriority = priorityLevels[userPreferences.priorityThreshold as keyof typeof priorityLevels] || 1;
      
      if (notificationPriority < thresholdPriority) {
        logger.debug(`Notification ${validatedNotification.id} below priority threshold for user ${validatedNotification.recipient}`);
      ...
      if (userPreferences.quietHoursEnabled && this.isQuietHours(userPreferences)) {
        ...
        if (validatedNotification.priority !== 'p1') {
          logger.debug(`Notification ${validatedNotification.id} delayed due to quiet hours`);
    ...
    logger.info(`Created notification ${validatedNotification.id} for ${validatedNotification.recipient}`);
    return validatedNotification;
  }
  ...
  async sendNotificationFromTemplate(
    ...
    return this.sendNotification({
      ...
      priority: (options?.priority as any) || 'p3',
      relatedEntityId: options?.relatedEntityId || '',
      relatedEntityType: (options?.relatedEntityType as any) || 'case',
  ...
  private async processNotification(notification: Notification): Promise<void> {
    ...
    for (const channel of notification.channels) {
      try {
        ...
        logger.error(`Failed to deliver notification ${notification.id} to ${channel}:`, error);
        allSucceeded = false;
      }
    }
  ...
  private async deliverToChannel(notification: Notification, channel: NotificationChannel): Promise<any> {
    const attempt = {
      id: uuidv4(),
      notificationId: notification.id,
      channel,
  ...
  private async sendSMS(notification: Notification): Promise<any> {
    ...
    logger.info(`Sending SMS to ${notification.recipient}: ${notification.message}`);
  ...
  private async sendSlack(notification: Notification): Promise<any> {
  ...
  private async sendTeams(notification: Notification): Promise<any> {
    ...
    try {
      const response = await axios.post(this.config.notifications.teamsConfig.webhookUrl, teamsMessage);
  ...
  private async sendWebhook(notification: Notification): Promise<any> {
  ...
  private async sendPagerDuty(notification: Notification): Promise<any> {
    // Mock PagerDuty integration
    logger.info(`Sending PagerDuty alert for ${notification.recipient}: ${notification.title}`);
  ...
  // Helper Methods
  private async recordDeliveryAttempt(attempt: DeliveryAttempt): Promise<void> {
  ...
  private async retryDelivery(attempt: DeliveryAttempt): Promise<void> {
    const notification = await this.getNotification(attempt.notificationId);
    if (!notification) return;

    logger.info(`Retrying delivery for notification ${attempt.notificationId}, attempt ${attempt.attempt}`);
    ...
    try {
      ...
      logger.error(`Retry failed for notification ${attempt.notificationId}:`, error);
    }
  }
