...
export class AlertClusteringService {
    ...
    constructor(vectorService, clusterStorage, config = this.DEFAULT_CONFIG) {
     ...
     */
    async clusterIncomingAlert(newAlert) {
     ...
     */
    async calculateSimilarityMatrix(alerts) {
     ...
     */
    async calculateAlertSimilarity(alert1, alert2) {
        // Text similarity (title and description)
        const titleSimilarity = await this.calculateTextSimilarity(alert1.title, alert2.title);
        const contentSimilarity = await this.calculateTextSimilarity(alert1.description, alert2.description);
        // Temporal similarity
        const temporalSimilarity = this.calculateTemporalSimilarity(alert1.timestamp, alert2.timestamp);
        ...
        const tacticSimilarity = this.calculateMitreSimilarity(alert1.mitreTactics, alert2.mitreTactics);
        const techniqueSimilarity = this.calculateMitreSimilarity(alert1.mitreTechniques, alert2.mitreTechniques);
        // Calculate weighted overall similarity
        const overallSimilarity = (titleSimilarity * this.config.weights.title) +
     ...
     */
    async applyClustering(alerts, similarityMatrix) {
     ...
     */
    dbscanClustering(alerts, similarityMatrix) {
        const n = alerts.length;
     ...
     */
    hierarchicalClustering(alerts, similarityMatrix) {
        const n = alerts.length;
        let clusters = alerts.map((_, i) => [i]); // Start with each alert as its own cluster
     ...
     */
    hybridClustering(alerts, similarityMatrix) {
        ...
        const clusteredAlertIds = new Set(dbscanClusters.flatMap(cluster => cluster.alerts.map(a => a.id)));
        ...
        const unclusteredIndices = unclusteredAlerts.map(alert => alerts.findIndex(a => a.id === alert.id));
     ...
     */
    createCluster(alerts, clusterId, method) {
        // Choose representative alert (highest risk score)
        const representativeAlert = alerts.reduce((prev, current) => current.riskScore > prev.riskScore ? current : prev);
     ...
     */
    calculateSpatialSimilarity(alert1, alert2) {
        const intersections = [
     ...
     */
    calculateIndicatorSimilarity(alert1, alert2) {
        const intersections = [
     ...
     */
    calculateMitreSimilarity(items1, items2) {
     ...
     */
    simpleStringSimilarity(str1, str2) {
     ...
     */
    getNeighbors(pointIndex, similarityMatrix, threshold) {
        const neighbors = [];
     ...
     */
    calculateIntraClusterSimilarity(alerts) {
        if (alerts.length < 2)
            return 1.0;
        let totalSimilarity = 0;
        let count = 0;
     ...
     */
    calculateClusterUrgency(alerts) {
        ...
        if (hasCritical && criticalRatio > 0.5)
            return 'critical';
     ...
     */
    generateClusterDescription(alerts) {
     ...
     */
    calculateClusteringConfidence(alerts, similarity) {
    ...
    async calculateAlertToClusterSimilarity(alert, cluster) {
...
//# sourceMappingURL=alert-clustering-service.js.map