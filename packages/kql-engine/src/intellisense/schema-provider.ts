    ...
    return [
      {
        ...
        columns: [
          ...
          { name: 'source', type: 'string', description: 'Log source system', nullable: false, examples: ['Windows', 'Syslog', 'AWS'] },
          { name: 'severity', type: 'string', description: 'Event severity level', nullable: false, examples: ['critical', 'high', 'medium', 'low', 'info'] },
          { name: 'category', type: 'string', description: 'Event category', nullable: false, examples: ['authentication', 'network', 'process', 'file'] },
          { name: 'message', type: 'string', description: 'Event message or description', nullable: true },
          { name: 'host_name', type: 'string', description: 'Source hostname', nullable: true },
          { name: 'host_ip', type: 'string', description: 'Source IP address', nullable: true },
          ...
          { name: 'metadata', type: 'dynamic', description: 'Event metadata', nullable: true },
          { name: 'enrichments', type: 'dynamic', description: 'Enrichment data', nullable: true },
          { name: 'risk_score', type: 'int', description: 'Risk score (0-100)', nullable: true, examples: [0, 25, 50, 75, 100] }
        ],
        sampleQueries: [
          'log_events | where severity == "critical"',
          'log_events | where timestamp > ago(1h)',
          'log_events | summarize count() by source',
          'log_events | where user_name contains "admin"'
        ]
      },
      {
