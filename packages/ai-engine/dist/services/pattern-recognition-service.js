 ...
 */
class PatternRecognitionService extends events_1.EventEmitter {
    constructor() {
        super();
        ...
        this.patternTimeout = 3600000; // 1 hour
    ...
    registerPattern(pattern) {
        ...
        logger_1.logger.info(`Registered attack pattern: ${pattern.name} (${pattern.mitreId || 'No MITRE ID'})`);
    }
    /**
        ...
        try {
            ...
            matches.forEach(match => {
                if (match.confidence > 0.7) {
                    ...
                    logger_1.logger.warn(`Attack pattern detected: ${match.patternId}`, {
            ...
            logger_1.logger.error('Error processing event for pattern recognition:', error);
            throw new ai_types_1.AIEngineError(`Failed to process event: ${error instanceof Error ? error.message : 'Unknown error'}`, 'PATTERN_PROCESSING_FAILED', { eventId: event.id, error: error instanceof Error ? error.message : 'Unknown error' });
        }
    }
    /**
    ...
    async checkPatternMatch(pattern, event) {
        const matches = [];
    ...
    getMatchingIndicators(pattern, event) {
        const matching = [];
    ...
    doesIndicatorMatch(indicator, event) {
        switch (indicator.type) {
            ...
            default:
                ...
                return Object.values(event.metadata).some(value => String(value).toLowerCase().includes(indicator.value.toLowerCase()));
        }
    }
    checkSequencePatterns(pattern, event, matchingIndicators, activeStates) {
        const matches = [];
    ...
    calculateRiskScore(pattern, confidence, stepsCompleted) {
        let baseScore = 0;
    ...
    mergePatternMatches(matches) {
        ...
        for (let i = 0; i < matches.length; i++) {
            ...
            for (let j = i + 1; j < matches.length; j++) {
                if (processed.has(j))
                    continue;
                const overlap = this.calculateEventOverlap(currentMatch.matchedEvents, matches[j].matchedEvents);
    ...
    calculateEventOverlap(events1, events2) {
    ...
    combineBomatches(match1, match2) {
        return {
            ...
            timeline: [...match1.timeline, ...match2.timeline].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()),
    ...
    setupCleanupRoutines() {
        ...
        setInterval(() => {
            const now = Date.now();
            for (const [patternId, states] of this.activePatterns) {
                const activeStates = states.filter(state => now - state.startTime <= this.patternTimeout);
    ...
    initializeBuiltInPatterns() {
        ...
        logger_1.logger.info('Initialized built-in attack patterns', {
            patternCount: this.attackPatterns.size
        });
    }
}
exports.PatternRecognitionService = PatternRecognitionService;
exports.default = PatternRecognitionService;
