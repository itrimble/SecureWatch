...
import { NaturalLanguageQuery, GeneratedQuery, SecurityContext, AIModelConfig, QueryType, AIEngineError } from '../types/ai.types';
 ...
 */
export class QueryGenerationService extends EventEmitter {
  private localProvider: LocalLLMProvider;
  private cloudProvider: CloudAIProvider;
  private queryExamples: Map<string, QueryExample[]> = new Map();
  private queryCache: Map<string, GeneratedQuery> = new Map();
   ...
   */
  async generateKQLFromNaturalLanguage(request: NaturalLanguageQuery): Promise<GeneratedQuery> {
    try {
      ...
      if (cached) {
        logger.debug('Returning cached query for:', request.question);
        return cached;
      }
      ...
      throw new AIEngineError(
        `Failed to generate KQL query: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'QUERY_GENERATION_FAILED',
        { question: request.question, error: error instanceof Error ? error.message : 'Unknown error' }
      );
    }
  }
...
`;
   ...
   */
  async explainQuery(query: string, context: SecurityContext): Promise<string> {
...
`;
  ...
  private selectAppropriateModel(request: NaturalLanguageQuery): AIModelConfig & { id: string } {
  ...
  private buildPromptWithContext(request: NaturalLanguageQuery): string {
...
Always respond with clean, executable KQL queries only.`;
  ...
  private getRelevantExamples(question: string): QueryExample[] {
    ...
    return allExamples
      .filter(example => 
        keywords.some(keyword => 
          example.natural.toLowerCase().includes(keyword) ||
  ...
  private getDataSourceInfo(dataSources: string[]): string {
  ...
  private extractKQLFromResponse(response: string): string {
    ...
    if (lines.length > 0) {
      ...
      const mainQuery = lines.find(line => 
        line.includes('|') || 
  ...
  private async validateKQLSyntax(query: string): Promise<KQLValidationResult> {
    const errors: string[] = [];
    const suggestions: string[] = [];
  ...
  private determineQueryComplexity(query: string): 'simple' | 'moderate' | 'complex' {
  ...
  private generateExplanation(question: string, query: string): string {
  ...
  private estimateResultSize(query: string): number | undefined {
  ...
  private createCacheKey(request: NaturalLanguageQuery): string {
    return `${request.question}:${request.context.privacyLevel}:${request.maxComplexity}:${JSON.stringify(request.context.timeRange)}`;
  }

  private logQueryGeneration(request: NaturalLanguageQuery, result: GeneratedQuery): void {
