...
import { AssessmentSchema, AssessmentResultSchema, QuizQuestionSchema } from '../types/educational.types';
export class AssessmentService extends EventEmitter {
    ...
    async submitQuestionResponse(sessionId, questionId, response) {
        ...
        switch (question.type) {
            ...
            case 'fill-blank':
                if (Array.isArray(question.correctAnswer)) {
                    return question.correctAnswer.some(correct => studentAnswer.toLowerCase().trim() === correct.toLowerCase().trim());
                }
    ...
    generateOverallFeedback(percentage, passed, passingScore) {
        if (passed) {
            if (percentage >= 90) {
    ...
    generateRecommendations(questionResults, assessment) {
    ...
    // Helper Methods
    getActiveSessionForStudent(assessmentId, studentId) {
    ...
    async getStudentAttempts(assessmentId, studentId) {
    ...
    async recordAnalyticsEvent(assessmentId, studentId, eventType, eventData) {
    ...
    async getAssessmentStatistics() {
        const [totalAssessments, totalAttempts, averageScoreResult, passRateResult, difficultyStats] = await Promise.all([
        ...
        return {
            ...
            passRate: totalAttempts?.count > 0
                ...
                : 0,
            popularAssessments: popularAssessments.map((row) => ({
                id: row.id,
                title: row.title,
                attempts: row.attempts,
                averageScore: Math.round(row.averageScore || 0)
            })),
            difficultyDistribution: difficultyStats.reduce((acc, row) => {
                acc[row.difficulty] = row.count;
                return acc;
            }, {}),
            questionTypeDistribution: questionTypeStats.reduce((acc, row) => {
