...
interface CertificationExam {
  ...
  maxAttempts: number;
  questions: string[]; // assessment IDs that make up the exam
  practicalTasks: string[]; // lab IDs for practical components
...
interface CertificationExamResult {
  ...
  assessmentResults: string[]; // assessment result IDs
  practicalResults: string[]; // lab result IDs
...
export class CertificationService extends EventEmitter {
  ...
  async searchCertifications(filters: SearchFilters, pagination: Pagination): Promise<{ certifications: Certification[], total: number, page: number, totalPages: number }> {
    ...
    const certifications: Certification[] = rows.map((row: any) => ({
      id: row.id,
      name: row.name,
      description: row.description,
      issuer: row.issuer,
  ...
  // Exam Management
  async createCertificationExam(examData: Omit<CertificationExam, 'id' | 'createdAt' | 'updatedAt'>): Promise<CertificationExam> {
    const now = new Date();
    const newExam: CertificationExam = {
      id: uuidv4(),
      createdAt: now,
      updatedAt: now,
      ...examData
  ...
  async startCertificationExam(examId: string, studentId: string): Promise<CertificationExamResult> {
    ...
    const now = new Date();
    const examResult: CertificationExamResult = {
      id: uuidv4(),
      examId,
      studentId,
  ...
  async submitCertificationExam(examResultId: string, assessmentResults: string[], practicalResults: string[]): Promise<CertificationExamResult> {
    ...
    const updatedResult: CertificationExamResult = {
      ...examResult,
      score: Math.round(totalScore),
      passed,
      completedAt: now,
  ...
  private async generateCertificate(examResultId: string): Promise<string> {
    const examResult = await this.getCertificationExamResult(examResultId);
  ...
  async verifyCertificate(certificateNumber: string): Promise<Certificate | null> {
    ...
    if (certificate.expiresAt && certificate.expiresAt < new Date()) return null;

    return certificate;
  }

  // Study Materials Management
  async createStudyMaterial(materialData: Omit<StudyMaterial, 'id' | 'createdAt' | 'updatedAt'>): Promise<StudyMaterial> {
    const now = new Date();
  ...
  async getStudyMaterials(certificationId: string): Promise<StudyMaterial[]> {
  ...
  // Helper Methods
  private async getCertificationExam(examId: string): Promise<CertificationExam | null> {
  ...
  private async getCertificationExamResult(examResultId: string): Promise<CertificationExamResult | null> {
  ...
  private async checkExamPrerequisites(examId: string, studentId: string): Promise<boolean> {
  ...
  private async getStudentExamAttempts(examId: string, studentId: string): Promise<CertificationExamResult[]> {
  ...
  private async calculateCompetencyScores(
    certificationId: string,
    studentId: string,
    assessmentResults: string[],
    practicalResults: string[]
  ): Promise<{ competencyId: string; score: number; weight: number }[]> {
  ...
  private generateExamFeedback(score: number, passed: boolean, competencyScores: any[]): string {
    if (passed) {
      if (score >= 90) {
  ...
  private generateCertificateNumber(certificationName: string, studentId: string): string {
  ...
  private generateDigitalSignature(certificateNumber: string, studentId: string): string {
  ...
  // Statistics
  async getCertificationStatistics(): Promise<CertificationStatistics> {
    const [
      totalCertifications,
      totalExams,
      totalCertificates,
