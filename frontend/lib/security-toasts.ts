...
export const securityToasts = {
  ...
  threatDetected: (threat: {
    ...
    config.fn(`${config.icon} ${threat.title}`, {
      ...
      action: threat.id
        ? {
            label: "Investigate",
            onClick: () => console.log(`Investigating threat/incident: ${threat.id}`), // Placeholder, ideally navigate
            // onClick: () => window.location.href = `/incidents/${threat.id}` // If you have incident pages
          }
        : undefined,
    })
  },
  ...
  // User Activity
  userActivity: (user: string, action: string, riskLevel: "low" | "medium" | "high") => {
    const riskConfig = {
  ...
  // Integration Status
  integrationStatus: (provider: string, status: "connected" | "disconnected" | "error") => {
    const statusConfig = {
    ...
    config.fn(`${config.icon} ${provider} Integration ${status}`, {
      ...
      action:
        status !== "connected"
          ? {
              label: "Settings",
              onClick: () => console.log(`Navigating to ${provider} settings...`), // Placeholder
