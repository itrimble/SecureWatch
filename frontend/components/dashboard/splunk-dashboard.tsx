...
const PANEL_TEMPLATES = [
  {
    ...
    visualization: 'line',
    icon: LineChart,
    defaultQuery: '* | bin timestamp as _time by 1h | stats count by _time | sort _time',
    defaultSize: { w: 6, h: 4 }
  },
  {
    id: 'top-sources',
  ...
  },
  {
    ...
    visualization: 'table',
    icon: Table,
    defaultQuery: 'event_type:security | head 100 | table timestamp, hostname, event_id, message',
    defaultSize: { w: 8, h: 6 }
  },
  {
    id: 'alert-count',
  ...
  },
  {
    ...
    icon: Activity,
    defaultQuery: '* | bucket timestamp span=1h | stats count by timestamp, source_ip',
    defaultSize: { w: 8, h: 5 }
  },
  {
    id: 'geo-threats',
    ...
    visualization: 'geo',
    icon: Globe,
    defaultQuery: '* | iplocation source_ip | stats count by Country | geom geo_countries',
    defaultSize: { w: 6, h: 5 }
  }
]
...
function SplunkDashboard() {
  ...
  const substituteTokens = useCallback((query: string, tokens: Record<string, any>) => {
    let substitutedQuery = query
    Object.entries(tokens).forEach(([token, value]) => {
      const tokenPattern = new RegExp(`\\$${token}\\$`, 'g')
      if (Array.isArray(value)) {
        substitutedQuery = substitutedQuery.replace(tokenPattern, value.map(v => `"${v}"`).join(' OR '))
      } else {
        substitutedQuery = substitutedQuery.replace(tokenPattern, value.toString())
      }
    })
  ...
  // Add new panel
  const addPanel = useCallback((template: typeof PANEL_TEMPLATES[0]) => {
  ...
  // Update panel
  const updatePanel = useCallback((panelId: string, updates: Partial<DashboardPanel>) => {
    setDashboardState(prev => ({
      ...prev,
  ...
  // Execute panel query
  const executePanel = useCallback(async (panel: DashboardPanel) => {
    updatePanel(panel.id, { isRefreshing: true })
    
    try {
      const substitutedQuery = substituteTokens(panel.query, dashboardState.tokens)
  ...
  const updateToken = useCallback((token: string, value: any) => {
    setDashboardState(prev => ({
      ...prev,
      tokens: { ...prev.tokens, [token]: value }
    }))
    // Auto-refresh panels if search on change is enabled
    if (dashboardState.inputs.find(input => input.token === token)?.searchOnChange) {
      setTimeout(refreshAllPanels, 100)
    }
  }, [dashboardState.inputs, refreshAllPanels])

  // Enhanced panel drill-down handler
  const handlePanelDrilldown = useCallback((panelId: string, drilldownData: any, event: React.MouseEvent) => {
  ...
  const handleNavigateToDashboard = useCallback((dashboardId: string, tokens?: Record<string, any>) => {
  ...
  const renderInput = useCallback((input: DashboardInput) => {
    ...
    switch (input.type) {
      case 'text':
        return (
          <div key={input.id} className="flex items-center space-x-2">
            <Label className="text-sm text-gray-300 whitespace-nowrap">{input.label}:</Label>
            <Input
              value={currentValue || ''}
              onChange={(e) => updateToken(input.token, e.target.value)}
              className="h-8 w-32 bg-gray-800 border-gray-600 text-gray-100"
      ...
      case 'dropdown':
        return (
          <div key={input.id} className="flex items-center space-x-2">
            <Label className="text-sm text-gray-300 whitespace-nowrap">{input.label}:</Label>
            <Select value={currentValue} onValueChange={(value) => updateToken(input.token, value)}>
              <SelectTrigger className="h-8 w-40 bg-gray-800 border-gray-600 text-gray-100">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        return (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              <div className="text-4xl font-bold text-blue-400">{singleValue.toLocaleString()}</div>
              <div className="text-sm text-gray-400 mt-1">{panel.title}</div>
            </div>
          </div>
        )
      default:
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400 mt-1">{dashboardState.description}</p>
          </div>
          ...
          <div className="flex items-center gap-2">
                    ...
                    >
                      <CardContent className="p-4">
                        <div className="flex items-start gap-3">
                          <template.icon className="w-8 h-8 text-blue-400 mt-1" />
                          <div>
            ...
            <Button
              ...
              size="sm"
              onClick={() => setDashboardState(prev => ({ ...prev, isEditMode: !prev.isEditMode }))}
            >
        ...
        {dashboardState.inputs.length > 0 && (
          <EnhancedInputControls
            ...
            onAutoRefreshToggle={(enabled) => setDashboardState(prev => ({ ...prev, autoRefresh: enabled }))}
            refreshInterval={dashboardState.refreshInterval}
            onRefreshIntervalChange={(interval) => setDashboardState(prev => ({ ...prev, refreshInterval: interval }))}
        ...
        {dashboardState.isEditMode && (
          <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4 mb-6">
            ...
            <p className="text-sm text-gray-400 mt-1">
              Drag panels to rearrange, resize by dragging corners, or click panels to edit their configuration.
            </p>
          </div>
        )}
          ...
          >
            {dashboardState.panels.map((panel) => (
              <Card
                ...
                className={`${
                  dashboardState.isEditMode 
                    ? 'border-dashed border-blue-500 cursor-move' 
                    : 'border-gray-700'
                } bg-gray-800 relative group transition-all duration-200`}
                onClick={() => dashboardState.isEditMode && setSelectedPanel(panel.id)}
              >
                <CardHeader className="pb-2 border-b border-gray-700">
                  ...
                  {panel.lastUpdated && (
                    <div className="text-xs text-gray-500">
                      Last updated: {format(new Date(panel.lastUpdated), 'HH:mm:ss')}
                    </div>
                  )}
                </CardHeader>
                <CardContent className="pt-2 h-full overflow-hidden">
                  <div 
                    className="h-full"
                    onClick={(e) => {
                      if (!dashboardState.isEditMode && panel.config.drilldown?.enabled) {
                        // Create mock drilldown data - in real implementation this would come from the visualization
                        const mockDrilldownData = {
                          value: panel.data?.[0]?.value || panel.data?.[0]?.count || 'unknown',
                          field: Object.keys(panel.data?.[0] || {})[0] || 'field',
        ...
        {dashboardState.panels.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-medium text-gray-300 mb-2">No Panels Added</h3>
            <p className="text-gray-500 mb-4">Start building your dashboard by adding some panels</p>
        ...
        <Dialog open={isPanelEditorOpen} onOpenChange={setIsPanelEditorOpen}>
          <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden p-0">
            <DialogHeader className="p-6 pb-2">
              <DialogTitle>
                {editingPanel ? `Edit Panel: ${editingPanel.title}` : 'Edit Panel'}
              </DialogTitle>
            </DialogHeader>
            {editingPanel && (
              <div className="px-6 pb-6">
                <PanelEditor
                  ...
                  onTestQuery={async (query) => {
                    const substitutedQuery = substituteTokens(query, dashboardState.tokens)
                    const response = await fetch('/api/v1/search/execute', {
                      method: 'POST',
                      headers: {
                        'Content-Type': 'application/json',
            ...
            <div className="px-6 pb-6">
              <InputsEditor
                ...
                onSave={(updatedInputs) => {
                  setDashboardState(prev => ({ ...prev, inputs: updatedInputs }))
