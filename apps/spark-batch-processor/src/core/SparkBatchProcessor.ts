...
export class SparkBatchProcessor extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        'Failed to initialize SparkBatchProcessor:',
        errorMessage
      );
      throw error;
    }
  ...
  private async createSparkSession(): Promise<any> {
    const sparkConfig: Record<string, string> = {
      ...
      // Kafka integration
      'spark.sql.streaming.kafka.consumer.pollTimeoutMs':
        this.config.kafka.kafkaConsumerPollTimeoutMs.toString(),
      'spark.sql.streaming.kafka.consumer.cache.capacity': '128',

      // Performance optimizations
      'spark.sql.adaptive.enabled':
        this.config.spark.enableAdaptiveQueryExecution.toString(),
      'spark.sql.adaptive.skewJoin.enabled':
        this.config.spark.adaptiveSkewJoin.toString(),
    ...
    if (this.config.kafka.securityProtocol !== 'PLAINTEXT') {
      sparkConfig['spark.kafka.security.protocol'] =
        this.config.kafka.securityProtocol;
      if (this.config.kafka.saslMechanism) {
        sparkConfig['spark.kafka.sasl.mechanism'] =
          this.config.kafka.saslMechanism;
        sparkConfig['spark.kafka.sasl.jaas.config'] =
          this.buildSaslJaasConfig();
      }
    }
    ...
    const spark = {
      ...
      writeStream: (df: any, options: any) =>
        this.createStreamWriter(df, options),
  ...
  async processHistoricalData(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(
        `Failed historical batch processing: ${batchId}`,
        errorMessage
      );
  ...
  async startMicroBatchProcessing(): Promise<void> {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(
        'Failed to start micro-batch processing:',
        errorMessage
      );
      throw error;
    }
   ...
   */
  private async readHistoricalData(
    options: BatchProcessingOptions
  ): Promise<any> {
    const kafkaOptions: Record<string, string> = {
    ...
    if (options.startDate || options.endDate) {
      kafkaOptions['startingOffsets'] = this.formatKafkaTimestampOffsets(
        options.startDate,
        options.endDate
      );
    }

    return await this.sparkSession.readStream
      .format('kafka')
  ...
  private async createKafkaStream(): Promise<any> {
    ...
    if (this.config.kafka.securityProtocol !== 'PLAINTEXT') {
      kafkaOptions['kafka.security.protocol'] =
        this.config.kafka.securityProtocol;
      if (this.config.kafka.saslMechanism) {
  ...
  private async writeProcessedData(
    ...
    if (this.config.batch.partitionBy.length > 0) {
      await dataFrame
        .partitionBy(...this.config.batch.partitionBy)
        .mode('append')
  ...
  private formatKafkaTimestampOffsets(
    startDate?: Date,
    endDate?: Date
  ): string {
