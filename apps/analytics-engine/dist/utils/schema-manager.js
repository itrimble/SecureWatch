...
export class SchemaManager {
    ...
    schemaCacheTTL = 300000; // 5 minutes in milliseconds
    constructor(dbPool) {
        this.dbPool = dbPool;
    }
    /**
     ...
     */
    async validateField(tableName, fieldName) {
     ...
     */
    async getFieldType(tableName, fieldName) {
        const tableSchema = await this.getTableSchema(tableName);
        if (!tableSchema) {
            return null;
        }
     ...
     */
    async getSuggestedFields(tableName, prefix = '') {
        const tableSchema = await this.getTableSchema(tableName);
        if (!tableSchema) {
            return [];
        }
        ...
        return allFields.filter(field => field.name.toLowerCase().startsWith(lowercasePrefix));
    }
    /**
     * Get schema mapping for KQL to SQL translation
     */
    async getSchemaMapping() {
        const schema = await this.getSchema();
        const mapping = {};
     ...
     */
    async loadTableFields(client, tableName) {
        const fieldsQuery = `
      SELECT 
        c.column_name,
        c.data_type,
     ...
     */
    async loadTableIndexes(client, tableName) {
        const indexQuery = `
      SELECT 
        i.indexname,
        i.indexdef,
     ...
     */
    mapPostgreSQLTypeToFieldType(pgType) {
     ...
     */
    findFieldInSchema(fields, fieldName) {
     ...
     */
    async getFieldExamples(client, tableName, fieldName) {
        try {
            const query = `
     ...
     */
    async isFieldIndexed(client, tableName, fieldName) {
        const query = `
      SELECT 1
      FROM pg_indexes
      WHERE tablename = $1
     ...
     */
    async loadJSONFieldChildren(client, tableName, fieldName) {
     ...
     */
    determineIndexType(indexDef) {
        if (indexDef.includes('gin'))
            return 'gin';
        if (indexDef.includes('gist'))
            return 'gist';
        if (indexDef.includes('hash'))
            return 'hash';
