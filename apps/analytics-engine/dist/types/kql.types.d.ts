...
export type QueryCategory = 'threat-detection' | 'ueba' | 'compliance' | 'network-analysis' | 'endpoint-security' | 'authentication' | 'data-exfiltration' | 'malware-analysis' | 'insider-threat' | 'vulnerability-management' | 'incident-response' | 'forensics' | 'threat-hunting' | 'asset-discovery' | 'performance-monitoring';
...
export type FilterOperator = 'eq' | 'neq' | 'gt' | 'gte' | 'lt' | 'lte' | 'contains' | 'startswith' | 'endswith' | 'matches' | 'in' | 'not_in' | 'between' | 'is_null' | 'is_not_null' | 'and' | 'or' | 'not';
...
export type AggregationFunction = 'count' | 'sum' | 'avg' | 'min' | 'max' | 'countif' | 'sumif' | 'avgif' | 'dcount' | 'stdev' | 'variance' | 'percentile' | 'median' | 'make_list' | 'make_set' | 'arg_min' | 'arg_max';
...
export type ExecutionStepType = 'table_scan' | 'index_scan' | 'filter' | 'aggregation' | 'sort' | 'join' | 'projection' | 'cache_lookup';
...
export type FieldType = 'string' | 'number' | 'boolean' | 'date' | 'timestamp' | 'ip_address' | 'email' | 'url' | 'hash' | 'uuid' | 'object' | 'array' | 'geo_location' | 'json';
...
export type KQLErrorType = 'syntax_error' | 'semantic_error' | 'execution_error' | 'timeout_error' | 'resource_error' | 'permission_error' | 'schema_error' | 'parameter_error';
...
//# sourceMappingURL=kql.types.d.ts.map