...
export class MFAService {
     ...
     */
    static async verifyTOTPSetup(userId, code) {
     ...
     */
    static async verifyTOTPCode(userId, code) {
    ...
    static async verifyBackupCode(userId, code) {
        ...
        const codeIndex = mfaMethod.recoveryCodes.findIndex(storedCode => storedCode === hashedCode);
        if (codeIndex === -1) {
            return false;
        }
        // Remove used backup code
        mfaMethod.recoveryCodes.splice(codeIndex, 1);
        await DatabaseService.updateMFABackupCodes(mfaMethod.id, mfaMethod.recoveryCodes);
        return true;
    }
    /**
     ...
     * Generate backup codes
     */
    static generateBackupCodes(count = authConfig.mfa.backupCodesCount) {
        const codes = [];
     ...
     */
    static async storePendingMFASetup(userId, data) {
     ...
     */
    static async getPendingMFASetup(userId) {
        const key = `mfa:pending:${userId}`;
     ...
     */
    static async clearPendingMFASetup(userId) {
        const key = `mfa:pending:${userId}`;
        await redis.del(key);
    }
}
