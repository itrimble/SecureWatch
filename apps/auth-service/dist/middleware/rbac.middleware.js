...
export const authorize = (requiredPermissions) => {
    ...
    return async (req, res, next) => {
        try {
            ...
            const hasAllPermissions = await PermissionService.checkPermissions(req.user.userId, req.user.organizationId, permissions);
 ...
 */
export const authorizeWithRoles = (requiredPermissions, requiredRoles) => {
    return async (req, res, next) => {
        // First check roles
 ...
 */
export const requireOrgAccess = (extractOrgId) => {
    return async (req, res, next) => {
        try {
            if (!req.user) {
                res.status(401).json({
