...
import { ParserError, ParserValidationError } from './types';
...
export class ParserManager {
    parsers = new Map();
    parsersBySource = new Map();
    parsersByCategory = new Map();
    communityLoader;
    validator;
    metrics;
    enrichment;
    ...
    async registerParser(parser) {
        try {
            // Validate parser implementation
            const validationResult = await this.validator.validateParser(parser);
            if (!validationResult.isValid) {
                throw new ParserValidationError(parser.id, `Validation failed: ${validationResult.errors.join(', ')}`);
            }
    ...
    async parseLog(rawLog, sourceHint, categoryHint) {
        ...
        try {
            ...
            for (const parser of candidates) {
                ...
                try {
                    ...
                    const parseTime = Date.now() - parseStartTime;
                    this.metrics.recordParseError(parser.id, parseTime, error);
            ...
            throw new ParserError('system', 'parsing', `System error: ${error.message}`, error);
        }
    }
    // Batch parse multiple logs
    async parseLogsAsync(logs, batchSize = 100) {
        const results = [];
        ...
        for (let i = 0; i < logs.length; i += batchSize) {
            ...
            const batchPromises = batch.map(async (logData, batchIndex) => {
                ...
                try {
                    const result = await this.parseLog(logData.rawLog, logData.sourceHint, logData.categoryHint);
                    return { index: actualIndex, result };
                }
                catch (error) {
                    logger.error(`Batch parse error at index ${actualIndex}:`, error);
                    return { index: actualIndex, result: null, error: error };
                }
            });
    ...
    // Get candidate parsers based on hints
    getCandidateParsers(sourceHint, categoryHint) {
    ...
    calculateConfidence(event, parser) {
