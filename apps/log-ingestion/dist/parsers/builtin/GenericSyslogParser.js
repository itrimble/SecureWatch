...
export class GenericSyslogParser {
    ...
    extractDeviceInfo(hostname) { if (!hostname)
        return undefined; return { name: hostname, hostname: hostname, type: 'server' }; }
    extractNetworkInfoFromJSON(jsonPayload) { if (!jsonPayload)
        return undefined; return { sourceIp: jsonPayload.src_ip || jsonPayload.source_ip || jsonPayload.client_ip, sourcePort: jsonPayload.src_port || jsonPayload.source_port, destinationIp: jsonPayload.dst_ip || jsonPayload.dest_ip || jsonPayload.server_ip, destinationPort: jsonPayload.dst_port || jsonPayload.dest_port || jsonPayload.server_port, protocol: jsonPayload.protocol, bytes: jsonPayload.bytes, packets: jsonPayload.packets }; }
    getCategoryFromFacility(facility) { const categoryMap = { 0: 'host', 1: 'host', 2: 'email', 3: 'host', 4: 'authentication', 5: 'host', 6: 'host', 7: 'host', 8: 'host', 9: 'host', 10: 'authentication', 11: 'file', 16: 'application', 17: 'application', 18: 'application', 19: 'application', 20: 'application', 21: 'application', 22: 'application', 23: 'application' }; return categoryMap[facility] || 'host'; }
    getActionFromMessage(message) { const lowerMessage = message.toLowerCase(); if (lowerMessage.includes('login') || lowerMessage.includes('logon'))
        return 'login'; if (lowerMessage.includes('logout') || lowerMessage.includes('logoff'))
        return 'logout'; if (lowerMessage.includes('failed') && lowerMessage.includes('auth'))
        return 'authentication_failed'; if (lowerMessage.includes('connection'))
        return 'connection'; if (lowerMessage.includes('disconnect'))
        return 'disconnection'; if (lowerMessage.includes('created'))
        return 'file_created'; if (lowerMessage.includes('deleted'))
        return 'file_deleted'; if (lowerMessage.includes('modified'))
        return 'file_modified'; if (lowerMessage.includes('started'))
        return 'process_started'; if (lowerMessage.includes('stopped'))
        return 'process_stopped'; if (lowerMessage.includes('error'))
        return 'error'; if (lowerMessage.includes('warning'))
        return 'warning'; if (lowerMessage.includes('info'))
        return 'info'; return 'unknown'; }
    getOutcomeFromMessage(message) { const lowerMessage = message.toLowerCase(); if (lowerMessage.includes('failed') || lowerMessage.includes('error') || lowerMessage.includes('denied') || lowerMessage.includes('rejected')) {
        return 'failure';
    } if (lowerMessage.includes('success') || lowerMessage.includes('accepted') || lowerMessage.includes('completed') || lowerMessage.includes('started')) {
        return 'success';
    } return 'unknown'; }
    mapToECSCategory(category) { const mapping = { 'authentication': ['authentication'], 'network': ['network'], 'host': ['host'], 'file': ['file'], 'email': ['email'], 'application': ['process'] }; return mapping[category] || ['host']; }
    mapToECSType(action) { const mapping = { 'login': ['start'], 'logout': ['end'], 'authentication_failed': ['start'], 'connection': ['connection'], 'disconnection': ['end'], 'file_created': ['creation'], 'file_deleted': ['deletion'], 'file_modified': ['change'], 'process_started': ['start'], 'process_stopped': ['end'], 'error': ['error'], 'warning': ['info'], 'info': ['info'] }; return mapping[action] || ['info']; }
    mapSeverityToNumber(severity) { const mapping = { 'low': 25, 'medium': 50, 'high': 75, 'critical': 100 }; return mapping[severity] || 25; }
    calculateConfidence(event) { let confidence = 0.6; if (event.custom?.jsonPayload) {
        confidence += 0.2;
    ...
    getTagsForEvent(facility, tag) { const baseTags = ['syslog', 'generic']; const facilityName = this.facilities[facility]; if (facilityName) {
