...
export class EnhancedJSONParser {
    ...
    extractUser(data) {
        ...
        for (const [field, paths] of Object.entries(userPaths)) {
            for (const path of paths) {
                const result = JSONPath({ path, json: data, wrap: false });
                if (result) {
                    user[field] = result.toString();
                    hasUserData = true;
                    break;
                }
            }
        }
        return hasUserData ? user : undefined;
    }
    extractDevice(data) {
        const devicePaths = {
        ...
        return hasDeviceData ? device : undefined;
    }
    extractNetwork(data) {
        const networkPaths = {
            ...
            for (const path of paths) {
                ...
                if (result) {
                    if (field.includes('Port')) {
                        network[field] = this.parseNumber(result);
                    }
                    else {
                        network[field] = result.toString();
                    }
                    hasNetworkData = true;
                    break;
                }
        ...
        return hasNetworkData ? network : undefined;
    }
    extractProcess(data) {
        const processPaths = {
        ...
        return hasProcessData ? process : undefined;
    }
    extractFile(data) {
        const filePaths = {
        ...
        return hasFileData ? file : undefined;
    }
    ...
    mapToECSType(data) {
        ...
        if (data.error)
            types.push('error');
        if (data.start || data.started)
            types.push('start');
        if (data.end || data.finished)
            types.push('end');
        if (data.change || data.update)
            types.push('change');
        if (data.create || data.created)
            types.push('creation');
        if (data.delete || data.deleted)
            types.push('deletion');
...
//# sourceMappingURL=EnhancedJSONParser.js.map