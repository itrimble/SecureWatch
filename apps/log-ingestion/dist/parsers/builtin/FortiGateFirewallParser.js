 ...
 */
export class FortiGateFirewallParser {
    ...
    validate(rawLog) {
        ...
        if (!this.hasKeyValueFormat(rawLog))
            return false;
        // Must contain type field
        if (!rawLog.includes('type='))
            return false;
    ...
    parse(rawLog) {
        try {
            ...
            const event = {
                ...
                action: action,
                outcome: outcome,
                severity: this.mapSeverity(fields.level),
                rawData: rawLog,
                custom: {
                    parsedFields: fields,
    ...
    normalize(event) {
        ...
        const normalized = {
            ...
            'securewatch.severity': this.mapSeverity(fields.level)
        };
    ...
    hasKeyValueFormat(rawLog) {
        // Check for key=value pattern
        const kvPattern = /\w+=("[^"]*"|\S+)/;
        return kvPattern.test(rawLog);
    }
    parseKeyValuePairs(rawLog) {
        const fields = {};
    ...
    parseTimestamp(date, time, eventtime) {
        if (eventtime) {
    ...
    mapCategory(logType) {
        if (!logType)
            return 'network';
        const typeConfig = this.logTypes[logType];
        return typeConfig?.category || 'network';
    }
    mapAction(action, eventtype, logType, subtype) {
        if (action)
    ...
    mapOutcome(action, eventtype) {
        const actionToCheck = (action || eventtype || '').toLowerCase();
        if (this.actionMappings[actionToCheck]) {
            return this.actionMappings[actionToCheck];
        }
    ...
    getEventCategory(logType, subtype) {
        switch (logType) {
...
//# sourceMappingURL=FortiGateFirewallParser.js.map