...
export class LEEFParser {
    ...
    version = '1.0.0';
    format = 'syslog';
    category = 'network'; // Default, can be overridden by LEEF fields
    priority = 90; // High priority for this specific format
    enabled = true;
    // LEEF header pattern
    leefPattern = /LEEF:[12]\.0\|/;
    validate(rawLog) {
    ...
    parse(rawLog) {
        ...
        let vendor, product, version, eventId, extensions, delimiter;
        let leefVersion;
        if (leef2Match) {
    ...
    parseLEEFExtension(extension, delimiter) {
        const fields = {};
    ...
    mapSeverity(severity) {
        ...
        if (lowerSeverity.includes('critical') || lowerSeverity.includes('fatal'))
            return 'critical';
        if (lowerSeverity.includes('high') || lowerSeverity.includes('error'))
            return 'high';
        if (lowerSeverity.includes('medium') || lowerSeverity.includes('warn'))
            return 'medium';
        if (lowerSeverity.includes('low') || lowerSeverity.includes('info'))
            return 'low';
        return 'medium';
    }
    ...
    mapCategory(category) {
        ...
        if (lowerCat.includes('auth'))
            return 'authentication';
        if (lowerCat.includes('file'))
            return 'file';
        if (lowerCat.includes('process'))
            return 'process';
        if (lowerCat.includes('network'))
            return 'network';
        if (lowerCat.includes('web'))
            return 'web';
        if (lowerCat.includes('malware'))
            return 'malware';
        return 'network';
    }
    getOutcome(fields) {
    ...
    mapToECSType(type) {
        ...
        if (lowerType.includes('start'))
            types.push('start');
        if (lowerType.includes('end') || lowerType.includes('stop'))
            types.push('end');
        if (lowerType.includes('access'))
            types.push('access');
        if (lowerType.includes('change') || lowerType.includes('modify'))
            types.push('change');
        if (lowerType.includes('create'))
            types.push('creation');
    ...
    mapDirection(direction) {
        ...
        if (lowerDirection.includes('in') || lowerDirection.includes('ingress'))
            return 'inbound';
        if (lowerDirection.includes('out') || lowerDirection.includes('egress'))
            return 'outbound';
        return undefined;
    }
    parseNumber(value) {
...
//# sourceMappingURL=LEEFParser.js.map