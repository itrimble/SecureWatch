...
export class AdaptiveBatchManager extends EventEmitter {
    ...
    recordBatchProcessing(batchSize, latency, throughput) {
        // Record metrics
    ...
    onBackpressureActivated() {
        if (this.config.adaptiveEnabled) {
            ...
            const newBatchSize = Math.max(this.config.minBatchSize, Math.floor(this.currentBatchSize * 0.7) // Reduce by 30%
            );
    ...
    onBackpressureDeactivated() {
        if (this.config.adaptiveEnabled) {
            ...
            const newBatchSize = Math.min(this.config.maxBatchSize, Math.floor(this.currentBatchSize * 1.1) // Increase by 10%
            );
    ...
    shouldAdjustBatchSize(avgLatency, avgThroughput) {
    ...
    calculateOptimalBatchSize(avgLatency, avgThroughput) {
    ...
    adjustBatchSize(newBatchSize, reason) {
    ...
    updateEvaluationMetrics(avgLatency, avgThroughput) {
    ...
    setBatchSize(size) {
        const constrainedSize = Math.max(this.config.minBatchSize, Math.min(this.config.maxBatchSize, size));
...
//# sourceMappingURL=adaptive-batch-manager.js.map