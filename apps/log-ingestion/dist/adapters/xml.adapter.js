...
export class XMLAdapter extends EventEmitter {
     ...
     */
    async processXMLString(xmlData, source = 'string', options = {}) {
        const mergedConfig = { ...this.config, ...options };
        const errors = [];
        let recordsProcessed = 0;
        try {
     ...
     */
    extractRecords(parsedData, config) {
        const records = [];
        if (config.recordPath) {
            ...
            if (Array.isArray(parsedData)) {
                ...
                if (arrayProps.length === 1) {
                    ...
                    const likelyCandidates = arrayProps.filter(key => key.toLowerCase().includes('log') ||
     ...
     */
    createLogEvent(record, source, index, config) {
...
//# sourceMappingURL=xml.adapter.js.map