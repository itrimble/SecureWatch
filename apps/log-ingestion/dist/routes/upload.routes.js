...
export class UploadRoutes {
    ...
    constructor(producerPool, bufferManager, metrics) {
        this.router = Router();
     ...
     * List all processing jobs
     */
    async listProcessingJobs(req, res) {
        try {
     ...
     */
    async processFileAsync(request) {
     ...
     * Process CSV file
     */
    async processCSVFile(request, status) {
        ...
        }, this.producerPool, this.bufferManager, this.metrics);
        await adapter.start();
        try {
     ...
     * Process XML file
     */
    async processXMLFile(request, status) {
     ...
     */
    async processJSONFile(request, status) {
     ...
     */
    async processTextFile(request, status) {
        const fileContent = await readFile(request.filePath, 'utf-8');
        const lines = fileContent.trim().split('\n');
        const errors = [];
        let eventsProcessed = 0;
     ...
     * Process EVTX file
     */
    async processEVTXFile(request, status) {
