...
export class CSVAdapter extends EventEmitter {
  ...
  private processedFiles: Map<string, CSVFileInfo> = new Map();
   ...
   */
  async processCSVString(csvData: string, source: string = 'string', options: Partial<CSVConfig> = {}): Promise<{
    success: boolean;
    rowsProcessed: number;
    errors: string[];
  }> {
   ...
   */
  private async parseCSVFile(filePath: string, config: CSVConfig): Promise<any[]> {
   ...
   */
  private async parseCSVString(csvData: string, config: CSVConfig): Promise<any[]> {
    return new Promise((resolve, reject) => {
      const rows: any[] = [];
   ...
   */
  private createLogEvent(row: any, source: string, index: number, config: CSVConfig): RawLogEvent {
