...
import { NormalizedLogEvent, LogSource, LogSeverity, LogCategory } from '../types/log-event.types';
...
export class SerializationBenchmark {
  ...
  private sampleEvents: Map<string, NormalizedLogEvent> = new Map();
  ...
  private generateSampleEvents(): void {
    ...
    const xlargeFields: Record<string, any> = { ...largeFields };
  ...
  async runBenchmark(config: BenchmarkConfig): Promise<Map<SerializationFormat, BenchmarkResults>> {
    ...
    const results = new Map<SerializationFormat, BenchmarkResults>();
  ...
  async runFullBenchmarkSuite(): Promise<void> {
    ...
    const eventSizes: Array<'small' | 'medium' | 'large'> = ['small', 'medium', 'large'];
