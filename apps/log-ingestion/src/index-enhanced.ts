...
let dbService: TimescaleDBService;
let enhancedProcessor: EnhancedLogProcessor;

// Adapters
let windowsAdapter: WindowsEventLogAdapter;
let syslogAdapter: SyslogAdapter;
let csvAdapter: CSVAdapter;
let xmlAdapter: XMLAdapter;
...
async function initializeServices() {
  try {
    ...
    enhancedProcessor = new EnhancedLogProcessor(
      dbService,
...
app.post('/api/logs/process', async (req, res) => {
  try {
    ...
    const results = await enhancedProcessor.processLogsBatch(
      logs.map(log => ({ rawLog: log, sourceHint, categoryHint }))
    );
