...
export class LogNormalizer {
  ...
  async normalize(rawEvent: any): Promise<NormalizedLogEvent> {
    try {
  ...
  private normalizeEVTXEvent(rawEvent: any): NormalizedLogEvent {
  ...
  private mapWindowsEventSeverity(eventId: number, level?: string): LogSeverity {
    ...
    if (level) {
      ...
      if (levelLower === 'critical') return LogSeverity.CRITICAL;
      if (levelLower === 'error') return LogSeverity.HIGH;
      if (levelLower === 'warning') return LogSeverity.MEDIUM;
      if (levelLower === 'information') return LogSeverity.INFO;
      if (levelLower === 'verbose') return LogSeverity.DEBUG;
    }

    return LogSeverity.INFO;
  }

  private classifyWindowsEventCategory(eventId: number, channel: string): LogCategory {
    // Security events
    if (channel === 'Security') {
  ...
  private buildWindowsEventMessage(eventId: number, eventData?: any): string {
    const descriptions: Record<number, string> = {
      4624: 'An account was successfully logged on',
      4625: 'An account failed to log on',
      4634: 'An account was logged off',
  ...
  private buildWindowsEVTXTags(event: any): string[] {
    const tags: string[] = ['windows', 'evtx'];
