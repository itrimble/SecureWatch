...
export class GenericSyslogParser implements LogParser {
  ...
  private extractDeviceInfo(hostname: string | null): DeviceInfo | undefined {
  ...
  private extractNetworkInfoFromJSON(jsonPayload: Record<string, any>): NetworkInfo | undefined {
  ...
  private getCategoryFromFacility(facility: number): string {
  ...
  private getActionFromMessage(message: string): string {
    ...
    if (lowerMessage.includes('login') || lowerMessage.includes('logon')) return 'login';
    if (lowerMessage.includes('logout') || lowerMessage.includes('logoff')) return 'logout';
    if (lowerMessage.includes('failed') && lowerMessage.includes('auth')) return 'authentication_failed';

    // Network actions
    if (lowerMessage.includes('connection')) return 'connection';
    if (lowerMessage.includes('disconnect')) return 'disconnection';

    // File actions
    if (lowerMessage.includes('created')) return 'file_created';
    if (lowerMessage.includes('deleted')) return 'file_deleted';
    if (lowerMessage.includes('modified')) return 'file_modified';

    // Process actions
    if (lowerMessage.includes('started')) return 'process_started';
    if (lowerMessage.includes('stopped')) return 'process_stopped';
  ...
  private getOutcomeFromMessage(message: string): 'success' | 'failure' | 'unknown' {
    const lowerMessage = message.toLowerCase();
  ...
  private getTagsForEvent(facility: number, tag?: string): string[] {
