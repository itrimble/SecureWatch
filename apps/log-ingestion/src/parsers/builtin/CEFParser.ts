 ...
 */
export class CEFParser implements LogParser {
  id = 'cef-parser';
  ...
  category = 'network' as const; // Default, can be overridden by CEF fields
  priority = 90; // High priority for this specific format
  enabled = true;

  // Simplified regex to find the CEF header
  private readonly cefPattern = /CEF:\d+\|/;
  ...
  private parseCEFExtension(extension: string): Record<string, any> {
  ...
  private mapSeverity(severity: string): 'low' | 'medium' | 'high' | 'critical' {
    ...
    if (isNaN(severityNum)) {
      ...
      if (lowerSeverity === 'very-high' || lowerSeverity === 'critical') return 'critical';
    }
  ...
  private mapDirection(direction?: number | string): 'inbound' | 'outbound' | undefined {
      if (direction === 0 || direction === 'inbound') return 'inbound';
      if (direction === 1 || direction === 'outbound') return 'outbound';
      return undefined;
  }
}