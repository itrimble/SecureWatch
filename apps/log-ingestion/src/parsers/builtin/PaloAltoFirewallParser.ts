 ...
 */
export class PaloAltoFirewallParser implements LogParser {
  ...
  validate(rawLog: string): boolean {
    ...
    if (!validLogTypes.includes(logType)) return false;
    ...
    if (receiveTime && !panTimeRegex.test(receiveTime)) return false;
    if (generateTime && !panTimeRegex.test(generateTime)) return false;
  ...
  parse(rawLog: string): ParsedEvent | null {
    try {
      ...
      const logType = fields[3]?.toUpperCase();
      const mapping = this.fieldMappings[logType as keyof typeof this.fieldMappings];
      ...
      const event: ParsedEvent = {
        ...
        outcome: this.mapOutcome(fields[30]) as 'success' | 'failure' | 'unknown',
        severity: this.mapSeverity(fields[33], logType) as 'low' | 'medium' | 'high' | 'critical',
        rawData: rawLog,
        custom: {
  ...
  normalize(event: ParsedEvent): NormalizedEvent {
    ...
    const normalized: NormalizedEvent = {
      ...
      'event.outcome': this.mapOutcome(fields.action) as 'success' | 'failure' | 'unknown',
      ...
      'network.direction': this.mapDirection(fields.direction) as 'ingress' | 'egress' | 'inbound' | 'outbound' | 'internal' | 'external' | 'unknown',
      ...
      'securewatch.severity': this.mapSeverity(fields.severity, logType) as 'low' | 'medium' | 'high' | 'critical'
    };
  ...
  private extractPanLogFromSyslog(rawLog: string): string {
  ...
  private extractFields(fields: string[], mapping: { [key: number]: string }): { [key: string]: any } {
  ...
  private calculateConfidence(fields: string[], logType: string): number {
    let confidence = 80; // Base confidence for PAN-OS logs

    // Higher confidence for complete logs
    if (fields.length > 30) confidence += 10;
    
    // Known log types get higher confidence
    const knownTypes = ['TRAFFIC', 'THREAT', 'SYSTEM'];
    if (knownTypes.includes(logType)) confidence += 5;
    
    // Valid timestamps increase confidence
    if (fields[1] && fields[6]) confidence += 5;

    return Math.min(confidence, 95);
  }

  private detectPanOsVersion(fields: string[]): string {
    // Heuristics to detect PAN-OS version based on field count and content
    const fieldCount = fields.length;
    
    if (fieldCount > 65) return '11.0+';
    if (fieldCount > 60) return '10.0+';
    if (fieldCount > 50) return '9.0+';
    return '8.0+';
  }

  private getEventCategory(logType?: string, fields?: any): string[] {
    switch (logType) {
  ...
  private getEventType(logType?: string, fields?: any): string[] {
    switch (logType) {
      case 'TRAFFIC':
  ...
  private mapOutcome(action?: string): string {
    if (!action) return 'unknown';
    
    const actionLower = action.toLowerCase();
    if (['allow', 'permit'].includes(actionLower)) return 'success';
    if (['deny', 'drop', 'block', 'reset'].includes(actionLower)) return 'failure';
    return 'unknown';
  }
  ...
  private mapDirection(direction?: string): string {
    ...
    if (dirLower.includes('client-to-server')) return 'outbound';
    if (dirLower.includes('server-to-client')) return 'inbound';
    return 'unknown';
  }

  private mapSeverity(severity?: string, logType?: string): string {
    if (!severity) return 'low';
    
    const sevLower = severity.toLowerCase();
    if (['critical', 'high'].includes(sevLower)) return 'high';
    if (['medium', 'informational'].includes(sevLower)) return 'medium';
    if (['low'].includes(sevLower)) return 'low';
    return 'low';
  }

  private mapSeverityToNumber(severity?: string, logType?: string): number {
    if (!severity) return 30;
    
    const sevLower = severity.toLowerCase();
    if (['critical'].includes(sevLower)) return 90;
    if (['high'].includes(sevLower)) return 70;
    if (['medium', 'informational'].includes(sevLower)) return 50;
    if (['low'].includes(sevLower)) return 30;
    return 30;
  }

  private mapThreatType(threatContentType?: string): string {
    if (!threatContentType) return 'unknown';
    
    const typeLower = threatContentType.toLowerCase();
    if (typeLower.includes('virus')) return 'file';
    if (typeLower.includes('spyware')) return 'malware-sample';
    if (typeLower.includes('vulnerability')) return 'vulnerability';
    return 'unknown';
  }

  private mapSystemLogLevel(eventId?: string): string {
    ...
    if (criticalEvents.some(e => eventId.includes(e))) return 'critical';
    if (errorEvents.some(e => eventId.includes(e))) return 'error';
    if (warningEvents.some(e => eventId.includes(e))) return 'warning';
    return 'info';
  }

  private mapToMitreAttack(threatName?: string, category?: string): any {
    if (!threatName && !category) return null;
  ...
  private extractVendorSpecificFields(fields: any, logType?: string): any {
    ...
    return Object.fromEntries(
      Object.entries(vendorFields).filter(([_, value]) => 
        value !== undefined && value !== null && value !== ''
      )
    );
  }

  private parseGenericPanLog(fields: string[], rawLog: string): ParsedEvent {
