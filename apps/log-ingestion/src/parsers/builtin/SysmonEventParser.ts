...
export class SysmonEventParser implements LogParser {
  id = 'sysmon-events';
  ...
  private readonly eventIdMapping: Record<number, string> = {
    1: 'Process Creation',
  ...
  private readonly suspiciousProcesses: Record<string, number> = {
    'mimikatz.exe': 0.95,
    'procdump.exe': 0.8,
    'psexec.exe': 0.7,
    'wce.exe': 0.9,
  ...
  parse(rawLog: string): ParsedEvent | null {
    try {
      ...
      if (!eventId || !this.eventIdMapping[eventId]) return null;
  ...
  private extractRuleName(xml: string): string | null {
  ...
  private extractProcessInfo(eventData: Record<string, any>): ProcessInfo {
    return {
      ...
      user: eventData.User,
      hashes: {
        md5: eventData.MD5 || eventData.Hashes?.split(',').find((h: string) => h.includes('MD5'))?.split('=')[1],
        sha1: eventData.SHA1 || eventData.Hashes?.split(',').find((h: string) => h.includes('SHA1'))?.split('=')[1],
        sha256: eventData.SHA256 || eventData.Hashes?.split(',').find((h: string) => h.includes('SHA256'))?.split('=')[1]
      },
  ...
  private extractNetworkInfo(eventData: Record<string, any>): NetworkInfo {
    return {
  ...
  private extractFileInfo(eventData: Record<string, any>): FileInfo {
  ...
  private extractRegistryInfo(eventData: Record<string, any>): RegistryInfo {
    return {
  ...
  private detectThreats(eventId: number, eventData: Record<string, any>, ruleName?: string | null): ThreatInfo | undefined {
  ...
  private detectFromRuleName(ruleName: string): { techniques: MitreTechnique[]; tactics: MitreTactic[] } | null {
    const ruleMapping: Record<string, { techniques: MitreTechnique[]; tactics: MitreTactic[] }> = {
      'technique_id=T1055': {
  ...
  private calculateEventConfidence(event: ParsedEvent): number {
  ...
  private getNetworkDirection(sourceIp: string, destIp: string): 'inbound' | 'outbound' | 'internal' | 'external' {
  ...
  private getSeverityForEventId(eventId: number, eventData: Record<string, any>, ruleName?: string | null): 'low' | 'medium' | 'high' | 'critical' {
    // Rule-based severity first
    if (ruleName && ruleName.includes('critical')) return 'critical';
    if (ruleName && ruleName.includes('high')) return 'high';
    if (ruleName && ruleName.includes('medium')) return 'medium';

    // Pattern-based severity
    const commandLine = eventData.CommandLine || '';
    if (this.attackPatterns.credential_dumping.pattern.test(commandLine)) return 'critical';
    if (this.attackPatterns.process_injection.pattern.test(commandLine)) return 'high';
    if (this.attackPatterns.uac_bypass.pattern.test(commandLine)) return 'high';
  ...
  private mapToECSCategory(category: string, eventId: number): string[] {
  ...
  private mapToECSType(eventId: number): string[] {
  ...
  private getTagsForEvent(eventId: number, ruleName?: string): string[] {
    ...
    const eventTags: Record<number, string[]> = {
      1: ['process-creation'],
      2: ['file-modification'],
      3: ['network-connection'],
  ...
  private extractRelatedIPs(event: ParsedEvent): string[] {
    const ips: string[] = [];
  ...
  private extractRelatedUsers(event: ParsedEvent): string[] {
    const users: string[] = [];
    if (event.process?.user) users.push(event.process.user);
    return users;
  }

  private extractRelatedHashes(event: ParsedEvent): string[] {
    const hashes: string[] = [];
