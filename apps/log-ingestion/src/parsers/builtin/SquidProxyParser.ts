// apps/log-ingestion/src/parsers/builtin/SquidProxyParser.ts
 ...
 */
export class SquidProxyParser implements LogParser {
  id = 'squid-proxy';
  name = 'Squid Proxy';
  vendor = 'Squid-Cache';
  ...
  private readonly squidRegex = /^(\d+\.\d+)\s+(\d+)\s+([\d\.]+)\s+([A-Z_]+)\/(\d+)\s+(\d+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\/(\S+)\s+(.+)$/;
  ...
  parse(rawLog: string): ParsedEvent | null {
    ...
    const [
        , timestamp, elapsed, clientIp, actionCode, status, size, 
        method, url, user, hierarchyHost, contentType
    ] = match;
