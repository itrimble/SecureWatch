 ...
 */
export class CiscoASAFirewallParser implements LogParser {
  ...
  validate(rawLog: string): boolean {
    ...
    if (isNaN(level) || level < 0 || level > 7) return false;

    // Validate message ID format (6 digits)
    const messageId = match[3];
    if (!/^\d{6}$/.test(messageId)) return false;
  ...
  parse(rawLog: string): ParsedEvent | null {
    try {
      ...
      const messagePattern = this.messagePatterns[messageId as keyof typeof this.messagePatterns];
      ...
      const event: ParsedEvent = {
        ...
        severity: this.mapSeverityFromLevel(level) as 'low' | 'medium' | 'high' | 'critical',
        rawData: rawLog,
        custom: {
          parsedFields: {
            device_type: device,
  ...
  normalize(event: ParsedEvent): NormalizedEvent {
    ...
    const normalized: NormalizedEvent = {
      ...
      'event.outcome': this.mapOutcome(event.action, fields) as 'success' | 'failure' | 'unknown',
      ...
      'securewatch.severity': this.mapSeverityFromLevel(fields.level) as 'low' | 'medium' | 'high' | 'critical'
    };
  ...
  private extractTimestamp(rawLog: string): Date {
  ...
  private parseMessageFields(messageId: string, messageText: string): any {
    const fields: any = {};
  ...
  private parseConnectionBuilt(messageText: string, fields: any): void {
  ...
  private parseConnectionTeardown(messageText: string, fields: any): void {
  ...
  private parseACLDeny(messageText: string, fields: any): void {
    ...
    if (match) {
      ...
      fields.dst = dstIp;
      fields.dport = dstPort;
      if (accessGroup) fields.access_group = accessGroup;
    }
  }

  private parseAuthentication(messageText: string, fields: any): void {
    ...
    if (match) {
      ...
      fields.dst = dstIp;
      fields.dport = dstPort;
      if (interfaceName) fields.src_interface = interfaceName;
    }
  }

  private calculateConfidence(messageId: string, fields: any): number {
    let confidence = 80; // Base confidence for ASA logs

    // Known message IDs get higher confidence
    if (this.messagePatterns[messageId as keyof typeof this.messagePatterns]) {
      confidence += 10;
    }
  ...
  private determineOutcome(messageId: string, messageText: string): 'success' | 'failure' | 'unknown' {
    const messagePattern = this.messagePatterns[messageId as keyof typeof this.messagePatterns];
  ...
  private mapSeverityFromLevel(level: string | number): string {
  ...
  private mapSeverityToNumber(level: string | number): number {
    const levelNum = typeof level === 'string' ? parseInt(level, 10) : level;
  ...
  private getEventCategory(category?: string): string[] {
    switch (category) {
      case 'connection': return ['network'];
      case 'access': return ['network'];
      case 'authentication': return ['authentication'];
  ...
  private getEventType(action?: string, fields?: any): string[] {
    if (!action) return ['info'];
    
    if (action.includes('built') || action.includes('start')) return ['start', 'connection'];
    if (action.includes('teardown') || action.includes('end')) return ['end', 'connection'];
    if (action.includes('deny')) return ['denied'];
    if (action.includes('allow')) return ['allowed'];
    if (action.includes('auth_success')) return ['start'];
    if (action.includes('auth_failure')) return ['start'];
    if (action.includes('threat')) return ['info'];
    
    return ['info'];
  }

  private mapOutcome(action: string, fields?: any): string {
  ...
  private mapDirection(direction?: string): 'ingress' | 'egress' | 'inbound' | 'outbound' | 'internal' | 'external' | 'unknown' {
    if (!direction) return 'unknown';
    
    const dirLower = direction.toLowerCase();
    if (dirLower === 'inbound') return 'inbound';
    if (dirLower === 'outbound') return 'outbound';
    return 'unknown';
  }
  ...
  private isAuthSuccess(messageId: string): boolean {
