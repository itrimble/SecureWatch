...
export class CommunityParserLoader {
  ...
  constructor(basePath: string = './community-rules') {
  ...
  async loadSigmaRules(): Promise<SigmaRule[]> {
    try {
      ...
      const cachedRules = await this.loadFromCache<SigmaRule[]>(cacheFile);
      if (cachedRules) {
      ...
      for (const file of files) {
        try {
          ...
          const parsed = yaml.load(content) as SigmaRule;
  ...
  async loadOSSECRules(): Promise<OSSECRule[]> {
    try {
      ...
      // Check if we have cached rules
      const cachedRules = await this.loadFromCache<OSSECRule[]>(cacheFile);
      if (cachedRules) {
  ...
  async loadElasticRules(): Promise<ElasticRule[]> {
    try {
      ...
      // Check if we have cached rules
      const cachedRules = await this.loadFromCache<ElasticRule[]>(cacheFile);
      if (cachedRules) {
      ...
      for (const file of files) {
        try {
          const content = await fs.readFile(file, 'utf-8');
          const parsed = JSON.parse(content) as ElasticRule | ElasticRule[];
  ...
  async downloadRules(): Promise<void> {
    try {
  ...
  private validateSigmaRule(rule: any): rule is SigmaRule {
    return (
      typeof rule === 'object' &&
  ...
  private validateElasticRule(rule: any): rule is ElasticRule {
    return (
      typeof rule === 'object' &&
  ...
  private async parseOSSECXml(xmlContent: string): Promise<OSSECRule[]> {
  ...
  private async loadFromCache<T>(cacheFile: string): Promise<T | null> {
    try {
  ...
  private async saveToCache<T>(cacheFile: string, data: T): Promise<void> {
    try {
  ...
  private async downloadSigmaRules(): Promise<void> {
    ...
    const sampleSigmaRule: SigmaRule = {
  ...
  private async downloadOSSECRules(): Promise<void> {
    // In a real implementation, you would:
  ...
  private async downloadElasticRules(): Promise<void> {
    // In a real implementation, you would:
    ...
    // Create sample rules for demonstration
    const sampleElasticRule: ElasticRule = {
