...
export class CorrelationEngine {
    ...
    async handlePatternMatch(pattern, event) {
        try {
            ...
            for (const matchedEvent of pattern.matched_events) {
                await this.incidentManager.addCorrelatedEvent(incident.id, matchedEvent.id, matchedEvent.timestamp, pattern.relevance_score);
            }
    ...
    extractAffectedAssets(event) {
        ...
        if (event.computer_name)
            assets.push(event.computer_name);
        if (event.user_name)
            assets.push(`user:${event.user_name}`);
        if (event.ip_address)
            assets.push(`ip:${event.ip_address}`);
        if (event.metadata?.target_host)
            assets.push(event.metadata.target_host);
        return [...new Set(assets)]; // Remove duplicates
    }
    async updateRuleMetrics(ruleId, success, executionTime) {
        try {
            const query = `
