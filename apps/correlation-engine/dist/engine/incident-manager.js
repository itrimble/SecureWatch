...
export class IncidentManager {
    ...
    async updateIncident(incidentId, event, result) {
        try {
            const query = `
        UPDATE incidents 
        SET 
    ...
    async findOpenIncident(ruleId, event, timeWindowMinutes) {
        try {
            const query = `
    ...
    async addCorrelatedEvent(incidentId, eventId, eventTimestamp, relevanceScore) {
        try {
            const query = `
    ...
    async getActiveIncidents(organizationId) {
        try {
            const query = `
        SELECT * FROM incidents 
    ...
    async updateIncidentStatus(incidentId, status, resolution) {
        try {
            const query = `
        UPDATE incidents 
        SET 
