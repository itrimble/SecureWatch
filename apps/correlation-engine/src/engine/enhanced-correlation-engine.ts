...
// Real-time performance enhancements for sub-second correlation
interface PerformanceMetrics {
  totalEventsProcessed: number;
  averageProcessingTime: number;
  ruleEvaluationCache: Map<string, { result: boolean; timestamp: number }>;
  eventBatchMetrics: {
    batchSize: number;
...
export class EnhancedCorrelationEngine {
  ...
  private fastQueue: PQueue; // High-priority queue for critical events
  ...
  private eventBatch: LogEvent[] = [];
  private batchTimer: NodeJS.Timeout | null = null;
  private indexedRules: Map<string, CorrelationRule[]> = new Map(); // Rules indexed by event type
  private bloomFilter: Set<string> = new Set(); // Simple bloom filter for fast rule filtering
  private processingTimes: number[] = []; // For P99 calculation
  ...
  private async addEventToBatch(event: LogEvent): Promise<void> {
  ...
  private async processBatch(): Promise<void> {
    ...
    try {
      ...
      const chunkPromises = chunks.map(chunk => 
        Promise.allSettled(chunk.map(event => this.processEventStream(event)))
      );
  ...
  private async loadAndIndexActiveRules(): Promise<void> {
    try {
      const query = `
        SELECT r.*, 
               array_agg(
  ...
  private generateIndexKeysForRule(rule: CorrelationRule): string[] {
  ...
  private getApplicableRulesForEvent(event: LogEvent): CorrelationRule[] {
    const applicableRules = new Map<string, CorrelationRule>();
  ...
  private async canUseFastPath(event: LogEvent): Promise<boolean> {
  ...
  private async processFastPath(event: LogEvent, context: CorrelationContext): Promise<void> {
    ...
    const matchPromises = results
      ...
      .map(result => this.handleRuleMatchAsync((result as any).value, event));
  ...
  private async evaluateRuleWithCache(rule: CorrelationRule, event: LogEvent): Promise<EvaluationResult> {
  ...
  private async processStandardPath(event: LogEvent, context: CorrelationContext): Promise<void> {
    ...
    if (evaluationResults.length > 0) {
      const incidentPromises = evaluationResults.map(result => 
        this.handleRuleMatchAsync(result, event).catch(error => {
  ...
  private addEventToOptimizedBuffer(event: LogEvent): void {
  ...
  private determineEventPriority(event: LogEvent): 'high' | 'normal' {
  ...
  private async handleRuleMatchAsync(result: EvaluationResult, event: LogEvent): Promise<void> {
    try {
  ...
  private extractAssetKey(event: LogEvent): string {
  ...
  private extractAffectedAssets(event: LogEvent): string[] {
    ...
    if (event.computer_name) assets.add(event.computer_name);
    if (event.user_name) assets.add(`user:${event.user_name}`);
    if (event.ip_address) assets.add(`ip:${event.ip_address}`);
    if (event.metadata?.target_host) assets.add(event.metadata.target_host);
  ...
  private adaptivePerformanceTuning(): void {
  ...
  private startOptimizedEventBufferCleanup(): void {
    setInterval(() => {
  ...
  async updateRealTimeConfig(config: Partial<RealTimeConfig>): Promise<void> {
  ...
  async enableStreamMode(): Promise<void> {
    this.realTimeConfig.streamProcessingMode = true;
