...
export class DatabaseService {
   ...
   */
  async upsertMarketplaceEntries(entries: MCPMarketplaceEntry[]): Promise<void> {
   ...
   * Upsert connection
   */
  async upsertConnection(connection: Omit<StoredConnection, 'created_at' | 'updated_at'>): Promise<void> {
    try {
      await this.pool.query(`
  ...
  async removeConnection(connectionId: string): Promise<void> {
    try {
      await this.pool.query(
        'UPDATE mcp_connections SET enabled = FALSE, updated_at = NOW() WHERE id = $1',
        [connectionId]
      )
