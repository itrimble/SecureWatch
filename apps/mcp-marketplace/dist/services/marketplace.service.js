import { MCPClient } from '@securewatch/mcp-client';
...
export class MCPMarketplaceService {
    ...
    dbService;
    cacheService;
    CACHE_KEY_MARKETPLACE = 'mcp:marketplace:entries';
    CACHE_KEY_CONNECTIONS = 'mcp:connections';
     ...
     */
    async discoverAndCache(aggregatorUrl) {
        try {
     ...
     */
    async installMCPServer(entryId) {
        try {
     ...
     */
    async uninstallMCPServer(connectionId) {
        try {
     ...
     */
    async invokeTool(connectionId, toolName, arguments_) {
        try {
    ...
    async getCachedMarketplaceEntries() {
        const cached = await this.cacheService.get(this.CACHE_KEY_MARKETPLACE);
        return cached || [];
    }
    async updateConnectionsCache() {
        const connections = await this.dbService.getConnections();
        await this.cacheService.set(this.CACHE_KEY_CONNECTIONS, connections, this.CACHE_TTL);
    }
    // Event handlers
    onDiscoveryComplete(entries) {
