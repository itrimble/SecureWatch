...
export function initializeJobRoutes(queue, executor, webSocket) {
    jobQueue = queue;
...
router.post('/submit', async (req, res) => {
    try {
        const { query, query_type = 'sql', parameters = {}, time_range, priority = 'normal', organization_id, user_id, metadata = {}, } = req.body;
        // Validation
        ...
        const estimatedDuration = await queryExecutor.estimateQueryDuration(query_type, query, parameters);
        // Create job object
        const job = {
            organization_id,
            user_id,
...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        const jobs = await jobQueue.getUserJobs(userId, status, parseInt(limit, 10));
        res.json({
            status: 'success',
            jobs,
            total: jobs.length,
