...
export class QueryExecutorService {
    ...
    async executeQuery(job, progressCallback) {
    ...
    async estimateQueryDuration(queryType, query, parameters) {
    ...
    async validateQuery(queryType, query) {
        const executor = this.executors.get(queryType);
        if (!executor) {
...
class SQLExecutor {
    ...
    async estimate_duration(query, parameters) {
        const client = await this.pgPool.connect();
        try {
...
class OpenSearchExecutor {
    ...
    async execute(job) {
        try {
            ...
            return {
                job_id: job.id,
                data: response.body.hits.hits.map((hit) => ({ _id: hit._id, ...hit._source })),
...
class KQLExecutor {
    ...
    convertKQLToSQL(kqlQuery, timeRange) {
