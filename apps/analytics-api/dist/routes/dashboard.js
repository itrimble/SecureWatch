// Specialized Dashboard Analytics API
...
let dbPool;
export function initializeDashboardRoutes(pool) {
    dbPool = pool;
}
// Real-time security overview (last 5 minutes)
router.get('/realtime-overview', async (req, res) => {
    ...
    `;
        ...
        const overview = {
            ...
            trend: {
                events_change: calculatePercentChange(latest.total_events, previous.total_events),
                errors_change: calculatePercentChange(latest.total_errors, previous.total_errors),
                sources_change: calculatePercentChange(latest.active_sources, previous.active_sources),
            },
...
router.get('/hourly-trends', async (req, res) => {
    ...
    `;
        ...
        dashboardCache.set(cacheKey, trends, 120); // Cache for 2 minutes
        res.json({
            status: 'success',
            data: trends,
            cached: false,
        ...
        logger.error('Failed to get hourly trends:', error);
        res.status(500).json({
            status: 'error',
...
router.get('/top-events', async (req, res) => {
    ...
    try {
      ...
      LIMIT $2;
    `;
        const result = await dbPool.query(query, [org_id || null, parseInt(limit, 10)]);
        const topEvents = {
            events: result.rows,
            period,
        ...
        dashboardCache.set(cacheKey, topEvents, 300); // Cache for 5 minutes
        res.json({
            status: 'success',
            data: topEvents,
            cached: false,
...
router.get('/source-health', async (req, res) => {
    ...
    `;
        ...
        dashboardCache.set(cacheKey, sourceHealth, 60); // Cache for 1 minute
        res.json({
            status: 'success',
            data: sourceHealth,
            cached: false,
...
router.get('/daily-summary', async (req, res) => {
    ...
    try {
      ...
      WHERE date_bucket >= CURRENT_DATE - INTERVAL '${parseInt(days, 10)} days'
        AND ($1::uuid IS NULL OR organization_id = $1::uuid)
    ...
    `;
        ...
        dashboardCache.set(cacheKey, dailySummary, 600); // Cache for 10 minutes
        res.json({
            status: 'success',
            data: dailySummary,
            cached: false,
...
router.get('/alert-performance', async (req, res) => {
    ...
    `;
        ...
        dashboardCache.set(cacheKey, alertPerformance, 180); // Cache for 3 minutes
        res.json({
            status: 'success',
        ...
        logger.error('Failed to get alert performance:', error);
        res.status(500).json({
            status: 'error',
